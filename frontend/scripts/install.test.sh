...
_setup_sandbox() {
  ...
  cat >"$payload_dir/multimira" <<'STUB'
#!/usr/bin/env bash
echo "multimira v0.3.2 (commit: test)"
STUB
  chmod +x "$payload_dir/multimira"
  tar -czf "$tmp/multimira.tar.gz" -C "$payload_dir" multimira

  cat >"$stub_bin/curl" <<'STUB'
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/multimira-ai/multimira/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if ! PATH="$tmp/stub-bin:$tmp/install-bin:/usr/bin:/bin" \
    MULTIMIRA_BIN_DIR="$tmp/install-bin" \
    MULTIMIRA_TEST_ARCHIVE="$tmp/multimira.tar.gz" \
  ...
  if [[ ! -x "$tmp/install-bin/multimira" ]]; then
    echo "expected fallback binary at $tmp/install-bin/multimira" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
