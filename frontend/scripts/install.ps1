# Multimira installer for Windows — one command to get started.
#
# Install CLI (default): connects to multimira.ai
#   irm https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.ps1 | iex
#
# Self-host: starts a local Multimira server + installs CLI + configures
#   $env:MULTIMIRA_MODE="local"; irm https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/multimira-ai/multimira.git"
$RepoWebUrl    = "https://github.com/multimira-ai/multimira"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".multimira\server"
$InstallDir    = if ($env:MULTIMIRA_INSTALL_DIR) { $env:MULTIMIRA_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/multimira-ai/multimira/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:MULTIMIRA_SELFHOST_REF) {
        return $env:MULTIMIRA_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = multimira version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing Multimira CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "Multimira requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/multimira-ai/multimira/releases/download/$latest/multimira-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "multimira-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "multimira.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/multimira-ai/multimira/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "multimira.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "multimira-cli-$version-windows-$arch.zip"
        $legacyAsset = "multimira_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "multimira.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".multimira\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "multimira.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "multimira.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "multimira.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "multimira.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "Multimira CLI installed to $binDir\multimira.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "multimira") {
        ...
        if ($isUpToDate) {
            Write-Ok "Multimira CLI is up to date ($currentVer)"
            return
        }

        Write-Info "Multimira CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "Multimira CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "multimira")) {
        Write-Fail "CLI installed but 'multimira' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. Multimira self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:MULTIMIRA_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up Multimira server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning Multimira repository..."
    ...
    Write-Info "Pulling official Multimira images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting Multimira services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "Multimira server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  Multimira - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] Multimira CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     multimira setup               " -NoNewline; Write-Host "# Connect to Multimira Cloud (multimira.ai)" -ForegroundColor DarkGray
    Write-Host "     multimira setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:MULTIMIRA_MODE="with-server"; irm https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  Multimira - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] Multimira server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     multimira setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:MULTIMIRA_MODE="stop"; irm https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping Multimira services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No Multimira installation found at $InstallDir"
    }

    if (Test-CommandExists "multimira") {
        try {
            multimira daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:MULTIMIRA_MODE) { $env:MULTIMIRA_MODE.ToLower() } else { "default" }
