...
require_config "$config" 'MULTIMIRA_APP_URL: http://localhost:3100'
...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "MULTIMIRA_APP_URL=${MULTIMIRA_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "MULTIMIRA_SERVER_URL=${MULTIMIRA_SERVER_URL}" \
...
require_env "$local_env" 'MULTIMIRA_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'MULTIMIRA_SERVER_URL=ws://localhost:9100/ws'
