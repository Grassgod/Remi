...
vi.mock("@multiremi/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@multiremi/core/onboarding")>(
      "@multiremi/core/onboarding",
    );
  return { ...actual, saveQuestionnaire: mockSaveQuestionnaire };
});

vi.mock("@multiremi/core/analytics", () => ({
...
describe("SourceBackfillModal", () => {
  it("does not render when there is no user", () => {
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Multiremi/i),
  ...
  it("opens for an onboarded user with empty source and fires the shown event", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Multiremi/i),
      ).toBeInTheDocument();
    });
