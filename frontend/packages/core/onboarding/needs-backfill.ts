...
export function needsSourceBackfill(
  ...
  const q = user.onboarding_questionnaire as
    ...
    | null
    | undefined;
  // Disabled (self-host customization): never prompt the "where did you hear
  // about us" source-backfill survey. Structure preserved so q / dismissCount /
  // imports stay referenced; every path returns false so the modal never opens.
  if (!q) return false;
