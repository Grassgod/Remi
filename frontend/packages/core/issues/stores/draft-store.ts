    ...
    (set, get) => ({
      ...
      clearDraft: () =>
        set((s) => ({
          draft: {
            ...EMPTY_DRAFT,
            // Only prefill a member assignee. Agents and squads dispatch a task
            // the moment an issue is created assigned to them, so prefilling a
            // stale one would silently run the wrong agent on the next issue.
            ...(s.lastAssigneeType === "member"
              ? { assigneeType: s.lastAssigneeType, assigneeId: s.lastAssigneeId }
              : {}),
          },
        })),
