...
Deploy Multimira on your own infrastructure in minutes.
...
Each user who runs AI agents locally also installs the **`multimira` CLI** and runs the **agent daemon** on their own machine.
...
This installs the `multimira` CLI, checks out the latest self-host assets, pulls the official Multimira images from GHCR, and configures everything for localhost.
...
`make selfhost-build` uses local `multimira-backend:dev` / `multimira-web:dev` tags, so it does not overwrite the pulled `:latest` images.
...
- **Deterministic local/private testing:** set `APP_ENV=development` and `MULTIMIRA_DEV_VERIFICATION_CODE=888888` in `.env`, then restart the backend. This fixed code is ignored when `APP_ENV=production`.
...
> **Warning:** do **not** set `MULTIMIRA_DEV_VERIFICATION_CODE` on a publicly reachable instance — anyone who knows an email address can then log in with that fixed code.
...
If you already run a Kubernetes cluster, you can deploy Multimira there instead of Docker Compose using the released OCI Helm chart at `oci://ghcr.io/multimira-ai/charts/multimira` or the source chart at [`deploy/helm/multimira/`](deploy/helm/multimira/). It targets a typical k3s / k8s setup with an Ingress controller and a default `ReadWriteOnce` StorageClass — authored against k3s + Traefik + `local-path`, and should work on any cluster with minor tweaks.

The chart creates the following resources in the target namespace:

- `multimira-postgres` — `pgvector/pgvector:pg17` backed by a 10Gi PVC
- `multimira-backend` — Go API/WS server. Backed by a 5Gi `ReadWriteOnce` uploads PVC by default; set `backend.uploads.persistence.enabled=false` when you have configured S3 (`backend.config.s3Bucket`) and don't want the chart to declare the PVC at all.
- `multimira-frontend` — Next.js standalone server
- Two `Ingress` resources: one for the web host, one for the backend host
- `multimira-config` ConfigMap (rendered from `values.yaml`)

The `multimira-secrets` Secret is **not** managed by the chart — you create it once with `kubectl` so real values never need to land in git.

> **One release per namespace:** the prebuilt `multimira-web` image bakes `REMOTE_API_URL=http://backend:8080` at build time, so the chart ships an ExternalName Service literally named `backend`. Because that name is unprefixed, you can run only one Multimira release per namespace, and `helm install` will fail if a `Service/backend` already exists there (pass `--take-ownership`, or use a dedicated namespace). If you build a web image with a patched `REMOTE_API_URL`, set `frontend.compatibility.backendAlias: false` to drop the alias.
...
The chart defaults to `multimira.dev.lan` (web) and `api.multimira.dev.lan` (backend). Pick one of:

- **`/etc/hosts`** on every machine that needs access (developer laptops + the machine running the daemon):

  ```text
  192.168.1.206  multimira.dev.lan api.multimira.dev.lan
  ```
...
```bash
kubectl create namespace multimira
```

### Step 3 — Create the `multimira-secrets` Secret

The chart references this Secret by name. Create it once with random values:

```bash
kubectl -n multimira create secret generic multimira-secrets \
  --from-literal=JWT_SECRET="$(openssl rand -hex 32)" \
  --from-literal=POSTGRES_PASSWORD="$(openssl rand -hex 16)" \
  --from-literal=RESEND_API_KEY="" \
  --from-literal=GOOGLE_CLIENT_SECRET="" \
  --from-literal=CLOUDFRONT_PRIVATE_KEY="" \
  --from-literal=MULTIMIRA_DEV_VERIFICATION_CODE=""
```
...
```bash
helm install multimira oci://ghcr.io/multimira-ai/charts/multimira \
...
```bash
helm show values oci://ghcr.io/multimira-ai/charts/multimira \
...
```bash
helm install multimira deploy/helm/multimira -n multimira
```

Watch the pods come up:

```bash
kubectl -n multimira get pods -w
```
...
```bash
curl -H "Host: api.multimira.dev.lan" http://<ingress-ip>/healthz
# {"status":"ok","checks":{"db":"ok","migrations":"ok"}}
```

Then open http://multimira.dev.lan in your browser.
...
- **Recommended (production):** patch the Secret with a real Resend key, then restart the backend:
  ...
  ```bash
  kubectl -n multimira patch secret multimira-secrets --type=merge \
    -p '{"stringData":{"RESEND_API_KEY":"re_xxx"}}'
  kubectl -n multimira rollout restart deploy/multimira-backend
  ```
...
- **Without email configured:** the verification code is generated server-side and printed to the backend pod logs (look for `[DEV] Verification code for ...:`). Useful for one-off testing.
  ...
  ```bash
  kubectl -n multimira logs -f deploy/multimira-backend | grep "Verification code"
  ```

- **Deterministic local/private testing:** set `backend.config.appEnv: development` in your values file and `MULTIMIRA_DEV_VERIFICATION_CODE=888888` in the Secret, then `helm upgrade` and restart. This fixed code is ignored when `APP_ENV=production`.

  ```bash
  helm upgrade multimira oci://ghcr.io/multimira-ai/charts/multimira \
    --version <chart-version> \
    -n multimira \
    -f my-values.yaml --set backend.config.appEnv=development
  kubectl -n multimira patch secret multimira-secrets --type=merge \
    -p '{"stringData":{"MULTIMIRA_DEV_VERIFICATION_CODE":"888888"}}'
  kubectl -n multimira rollout restart deploy/multimira-backend
  ```
...
```bash
multimira setup self-host \
  --server-url http://api.multimira.dev.lan \
  --app-url http://multimira.dev.lan
```
...
```bash
kubectl -n multimira rollout restart deploy/multimira-backend deploy/multimira-frontend
```

To upgrade to a specific Multimira release, upgrade to the matching chart version. The released chart defaults its app images to the matching Git tag:
...
```bash
helm -n multimira rollback multimira
```

> **Upgrading from `v0.3.4` to `v0.3.5+` fails with `refusing to drop legacy daily rollups: ...`?** As of MUL-2957 the `migrate up` command runs an idempotent monthly-slice backfill automatically before applying migration `103`, so a clean upgrade is a single `helm upgrade` + backend rollout. If you are still on a pre-MUL-2957 binary or the auto-hook fails, run the standalone backfill against the same database the chart is using (`kubectl -n multimira exec deploy/multimira-backend -- ./backfill_task_usage_hourly --sleep-between-slices=2s`), then restart the backend deployment to re-apply migrations. See [Advanced Configuration → Usage Dashboard Rollup](SELF_HOSTING_ADVANCED.md#usage-dashboard-rollup) for the full recovery flow.

### Tearing down

```bash
# Remove the workloads but keep the PVCs and the Secret
helm -n multimira uninstall multimira

# Wipe everything, including PostgreSQL data and uploads
kubectl delete namespace multimira
```
...
3. Leave the `pg_cron` extension itself installed unless you are sure no other workload depends on it. The bundled `pgvector/pgvector:pg17` image does **not** ship `pg_cron`, so nothing in Multimira's default install needs it; uninstalling `pg_cron` from a custom image that other workloads still use is a separate decision.
...
```bash
curl -fsSL https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.sh | bash -s -- --stop
```
...
## Switching to Multimira Cloud

If you've been self-hosting and want to switch your CLI to [Multimira Cloud](https://multimira.ai):

```bash
multimira setup
```

This reconfigures the CLI for multimira.ai, re-authenticates, and restarts the daemon. You will be prompted before overwriting the existing configuration.
...
Pin `MULTIMIRA_IMAGE_TAG` in `.env` to an exact version like `v0.2.4` if you want to stay on a specific release. Migrations run automatically on backend startup.
...
If you prefer configuring the CLI step by step instead of `multimira setup`:
