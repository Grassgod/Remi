...
function browserStore() {
  ...
  return {
    ...
    store: createAuthStore({
      ...
      revokeCookieOnLogout: props.revokeCookieOnLogout,
...
describe("WebProviders logout", () => {
  it("does not revoke cookie auth when initialization only clears the logged-in marker", () => {
    const fetchMock = vi.fn().mockResolvedValue(json({ message: "logged out" }));
    vi.stubGlobal("fetch", fetchMock);
    browserStore();
    // AuthInitializer invokes this hook when no bearer token is stored.
    captured.props?.onLogout?.();
