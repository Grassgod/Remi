...
describe("CallbackPage", () => {
  ...
  // Onboarding funnel disabled (self-host customization): the callback delegates
  // the default destination to `resolvePostAuthDestination`, which never routes
  // to the /onboarding wizard. An un-onboarded user with no pending invitations
  // and no workspace therefore lands on the create-workspace page. See
  // `@multiremi/core/paths/resolve.ts` (and its resolve.test.ts).
  ...
  // Onboarding funnel disabled: a listMyInvitations error is non-fatal and
  // falls through to the resolved default destination, which is the
  // create-workspace page (never /onboarding) for an un-onboarded user with
  // no workspace.
