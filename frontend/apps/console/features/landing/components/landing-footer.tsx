...
import { captureDownloadIntent } from "@multiremi/core/analytics";
import { XMark, GitHubMark, githubUrl, twitterUrl } from "./shared";
import { useLocale, locales, localeLabels } from "../i18n";

// Marketing routes that no longer exist in this internal self-hosted
// build. The footer link copy still lists them across all locales, so we
// filter them out at render time rather than editing four i18n bundles.
const REMOVED_LINK_HREFS = new Set(["/usecases", "/download", "/about", "/contact-sales"]);
...
export function LandingFooter() {
  ...
  const groups = Object.values(t.footer.groups).map((group) => ({
    ...group,
    links: group.links.filter((link) => !REMOVED_LINK_HREFS.has(link.href)),
  }));
