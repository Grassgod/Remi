...
import { LOCALE_COOKIE } from "@multiremi/core/i18n";
import {
...
// Forward the resolved locale to RSC layouts via the `x-multimira-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(MULTIMIRA_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("multimira_logged_in");
