.PHONY: help makehelp dev server server-bun daemon daemon-bun cli multimira build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop
...
POSTGRES_DB ?= multimira
POSTGRES_USER ?= multimira
POSTGRES_PASSWORD ?= multimira
POSTGRES_PORT ?= 5432
PORT := $(or $(BACKEND_PORT),$(API_PORT),$(SERVER_PORT),$(PORT),8080)
FRONTEND_PORT ?= 3000
FRONTEND_ORIGIN ?= http://localhost:$(FRONTEND_PORT)
MULTIMIRA_APP_URL ?= $(FRONTEND_ORIGIN)
DATABASE_URL ?= postgres://$(POSTGRES_USER):$(POSTGRES_PASSWORD)@localhost:$(POSTGRES_PORT)/$(POSTGRES_DB)?sslmode=disable
NEXT_PUBLIC_API_URL ?= http://localhost:$(PORT)
NEXT_PUBLIC_WS_URL ?= ws://localhost:$(PORT)/ws
GOOGLE_REDIRECT_URI ?= $(FRONTEND_ORIGIN)/auth/callback
MULTIMIRA_SERVER_URL ?= ws://localhost:$(PORT)/ws
...
selfhost: ## Create .env if needed, then pull and start the official self-hosted images
	...
	@echo "==> Pulling official Multimira images..."
	@if ! docker compose -f docker-compose.selfhost.yml pull; then \
		echo ""; \
		echo "Official images for tag '$${MULTIMIRA_IMAGE_TAG:-latest}' are not published yet."; \
		echo "If this is before the first GHCR release, build from the current checkout:"; \
		echo "  make selfhost-build"; \
		exit 1; \
	fi
	@echo "==> Starting Multimira via Docker Compose..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		echo ""; \
		echo "✓ Multimira is running!"; \
		echo "  Frontend: http://localhost:$${FRONTEND_PORT:-3000}"; \
		echo "  Backend:  http://localhost:$${PORT:-8080}"; \
		echo ""; \
		echo "Images: $${MULTIMIRA_BACKEND_IMAGE:-ghcr.io/multimira-ai/multimira-backend}:$${MULTIMIRA_IMAGE_TAG:-latest}"; \
		echo "        $${MULTIMIRA_WEB_IMAGE:-ghcr.io/multimira-ai/multimira-web}:$${MULTIMIRA_IMAGE_TAG:-latest}"; \
		echo ""; \
		echo "Log in: configure RESEND_API_KEY in .env for email codes,"; \
		echo "        or read the generated code from backend logs when Resend is unset."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install multimira-ai/tap/multimira"; \
		echo "  multimira setup self-host"; \
	else \
		echo ""; \
...
selfhost-build: ## Build backend/web from the current checkout and start the self-hosted stack
	...
	@echo "==> Building Multimira from the current checkout..."
	...
	@if curl -sf http://localhost:$${PORT:-8080}/health > /dev/null 2>&1; then \
		...
		echo "Local tags: multimira-backend:dev and multimira-web:dev."; \
		echo ""; \
		echo "Next — install the CLI and connect your machine:"; \
		echo "  brew install multimira-ai/tap/multimira"; \
		echo "  multimira setup self-host"; \
...
selfhost-stop: ## Stop the self-hosted Docker Compose stack
	@echo "==> Stopping Multimira services..."
...
server-bun: ## Run the Bun server for the current checkout (reads $(ENV_FILE))
	$(REQUIRE_ENV)
	@bash scripts/ensure-postgres.sh "$(ENV_FILE)"
	@cd server-bun && { [ -d node_modules ] || bun install; }
	@set -a; . ./$(ENV_FILE); set +a; cd server-bun && bun run src/main.ts

daemon-bun: ## Run the Bun agent daemon: make daemon-bun RUNTIME_ID=<uuid>
	$(REQUIRE_ENV)
	@if [ -z "$(RUNTIME_ID)" ]; then echo "Usage: make daemon-bun RUNTIME_ID=<runtime uuid> (create a runtime in the UI first)"; exit 1; fi
	@cd server-bun && { [ -d node_modules ] || bun install; }
	@set -a; . ./$(ENV_FILE); set +a; cd server-bun && RUNTIME_ID=$(RUNTIME_ID) bun run src/daemon-main.ts

daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) multimira MULTIMIRA_ARGS="daemon restart --profile local"

cli: ## Run the multimira CLI with ARGS or MULTIMIRA_ARGS from source
	@$(MAKE) multimira MULTIMIRA_ARGS="$(MULTIMIRA_ARGS)"

multimira: ## Run the multimira CLI entrypoint directly from the Go source tree
	cd server && go run ./cmd/multimira $(MULTIMIRA_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/multimira ./cmd/multimira
