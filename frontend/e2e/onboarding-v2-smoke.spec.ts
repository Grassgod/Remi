...
test("onboarding v2 — welcome → source → role → use_case (skip path)", async ({ page }) => {
  ...
  await page.evaluate((t) => {
    localStorage.setItem("multimira_token", t);
  }, token);
  ...
  // 2. Source step
  await expect(page.getByText("How did you hear about Multimira?")).toBeVisible({ timeout: 10000 });
  ...
  // 4. Use case step
  await expect(page.getByText("What do you want to use Multimira for?")).toBeVisible({ timeout: 10000 });
...
test("onboarding v2 — rage-skip all 3 questions", async ({ page }) => {
  ...
  await page.goto("/login");
  await page.evaluate((t) => localStorage.setItem("multimira_token", t), token);
  await page.goto("/onboarding");
  await page.waitForLoadState("networkidle");
...
test("onboarding v2 — zh-Hans renders Chinese labels", async ({ page, context }) => {
  await context.addCookies([
    { name: "multimira-locale", value: "zh-Hans", url: "http://localhost:13442" },
  ]);
  const api = new TestApiClient();
  ...
  await expect(page.getByText("你是从哪里了解到 Multimira 的？")).toBeVisible({ timeout: 10000 });
  await page.waitForTimeout(500);
