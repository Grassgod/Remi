...
- **Mobile (`apps/mobile/`) is independent.** It shares only **types and pure functions** from `@multiremi/core/`, with `import type` for types (zero runtime coupling). UI, state, hooks, providers, i18n, React version, build pipeline, release cadence — all mobile-owned.
...
pnpm --filter @multiremi/desktop build      # Compile TS → JS (reads .env.production)
pnpm --filter @multiremi/desktop package    # Package into .app/.dmg/.exe (current platform only)
...
- `packages/ui/` — zero `@multiremi/core` imports (pure UI, no business logic).
...
Rules for `apps/mobile/` live in `apps/mobile/CLAUDE.md`. Read it before touching anything in `apps/mobile/` — it covers what may be imported from `@multiremi/core/`, the React version policy, the build/release pipeline, and the locked tech-stack baseline.
...
Every full-window desktop view (anything outside the dashboard shell) must mount `<DragStrip />` from `@multiremi/views/platform` as the first flex child of the page root, otherwise users can't drag the window. Interactive UI inside the top 48px needs `WebkitAppRegion: "no-drag"` to stay clickable.
...
**Never test shared component behavior in an app's test file.** If a test requires mocking `next/navigation` or `react-router-dom` to test a component from `@multiremi/views`, the test is in the wrong place — move it to `packages/views/` and mock `@multiremi/core` instead.
...
- Mock `@multiremi/core` stores with `vi.hoisted()` + `Object.assign(selectorFn, { getState })` pattern (Zustand stores are both callable and have `.getState()`).
- Mock `@multiremi/core/api` for API calls.
