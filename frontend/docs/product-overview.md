...
> **文档说明**
>
> 这份文档的目的是：**让任何没有写过代码的新同事，在 30 分钟内完全理解 Multimira 这个产品到底有哪些功能、每个功能在整体中处于什么位置、一个功能和另一个功能如何协同**。
>
> 它的受众包括：
>
> - **新加入的工程师 / 产品 / 设计 / 运营**——用它做 onboarding 的第一份材料
> - **产品介绍工作**——需要对外讲解 Multimira 时的事实基础
...
1. [Multimira 是什么](#1-multimira-是什么)
2. [核心概念词典](#2-核心概念词典)
...
> Multimira 不是一个 AI 工具，而是一个**人 + AI 协作的任务管理平台**。agent 是一等公民，和人在同一个工作流里。

### 部署形态

- **云版本（Multimira Cloud）**：官方托管服务，agent 通过你本地跑的 daemon 执行
- **自托管（Self-Host）**：完整后端可以部署在自己的服务器
- **客户端**：Next.js web 版 + Electron 桌面版（两端体验基本一致，桌面独有：多标签、原生托盘、自动更新）

### 支持的 Coding Agent

Multimira **不自己训模型**，也不锁定某一家厂商。它是调度器，本地 daemon 会自动探测以下 CLI 工具并接入：
...
> **角色**：Multimira 的核心工作对象。

Issue 对应的概念在 Linear 叫 Issue、在 Jira 叫 Ticket、在 GitHub 叫 Issue——就是一个任务单元。Multimira 的特色在于**issue 可以分配给 agent，和分配给人完全对等**。
...
- **默认执行者（default assignee）**：项目可以绑定一个默认执行者（member / agent / squad 干活组），与 Lead 负责人（汇报人，通常是真人）是两个独立概念。手动创建 issue 时选中项目，assignee 会自动预填成这个默认值（之后仍可手动修改）。通过智能体创建时，创建者保持为上次使用的 agent/squad，项目只作为可选归属上下文；未选择项目时，创建者会检查现有项目并判断 issue 的归属。创建者和执行者不能混为同一个选择器语义。
- 详情页展示项目内的所有 issue
- 支持搜索项目
...
Agent 是 Multimira 的灵魂。几乎所有功能都围绕"如何让一个 agent 干活"展开：
- Issue 通过分配触发 agent
- Skill 通过挂载赋能 agent
...
这是 Multimira **分布式执行架构**的核心设计：**agent 不在 server 上运行，而在用户自己的机器上运行**。Server 只做任务调度、状态同步、数据存储。

#### Daemon 是什么

`multimira` CLI 在用户的机器上启动一个后台进程（macOS launchd / Linux systemd / Windows 服务风格），它：
...
| 命令 | 说明 |
|------|------|
| `multimira setup` | 一键配置：填 URL + 登录 + 启动 daemon |
| `multimira login` | 浏览器打开 OAuth 登录，保存 90 天 PAT 到 `~/.multimira/config.json` |
| `multimira login --token <pat>` | 无头登录（SSH/CI） |
| `multimira daemon start` | 后台启动 daemon（写 PID 到 `~/.multimira/daemon.pid`，日志到 `~/.multimira/daemon.log`） |
| `multimira daemon stop` | 发 SIGTERM，优雅关闭（等待进行中的任务完成，超时 30s） |
| `multimira daemon status` | 打印 daemon 状态、探测到的 agent、watch 中的 workspace |
| `multimira daemon logs -f` | 实时跟随日志 |
| `multimira daemon start --profile <name>` | 启动独立配置的 daemon（用于多环境，比如同时连 staging 和生产） |

#### 安全边界

- 每个任务一个**独立工作目录** `~/multimira_workspaces/{ws}/{task_short_id}/workdir/`
- 环境变量**过滤**：阻止 agent 覆盖 daemon 的认证变量（`MULTIMIRA_TOKEN` 等）
...
```bash
multimira skill list
multimira skill get <id>
multimira skill create --title ...
multimira skill import --url https://...
multimira skill files upsert <skill-id> --path ...
```

#### 产品里的位置

Skill 是 Multimira 区别于"每次都要写长 prompt"的关键机制。它让团队的专业知识**沉淀成可复用的组件**，绑在 agent 上就生效——就像给员工写的 SOP/playbook。
...
- **API / Manual**：UI 上点"立即运行"按钮，或用 CLI `multimira autopilot trigger <id>`
...
Autopilot 让 Multimira 从"你分配 → agent 做"升级到"agent 自己发起工作"。配合 `run_only` 模式，甚至可以在没有 issue 的前提下跑定时任务。Issue 上的 `origin_type=autopilot` + `origin_id` 字段留下了"这个 issue 是哪个 autopilot run 创建的"的追溯链。
...
桌面端的 `multimira://invite/{id}` 深链接**不是走路由**，而是触发 `WindowOverlay`——共享视图组件 `InvitePage` 装在原生窗口覆盖层里，保证拖拽移动窗口等原生体验。

#### 产品里的位置

成员管理是**一切协作的前提**。但在 Multimira 里它有一个独特之处：成员系统也管 agent。之所以要有 `assignee_type` 区分 member 和 agent，就是为了让两者在同一套 API 里表达"谁可以被分配"。
...
Settings 是所有"配置即工作"动作的汇总：agent 的 prompt、workspace 的 context、仓库白名单、skill 的内容——都在这里。**对运营和文案来说最重要的一句话**：用户在 Multimira 的 settings 页面做的配置，每一项都会影响 agent 实际执行时读到的上下文。

---

### 3.14 CLI 命令行工具

`multimira` 不只是启动 daemon 的工具，也是完整的命令行操作层。很多用户喜欢在终端里推进工作而不是开 UI。

#### 工作区 / 议题

```bash
multimira workspace list | get | watch | unwatch
multimira issue list | get | create | update | assign | status
multimira issue comment list | add | delete
multimira issue runs <id>                 # 查看任务执行历史
multimira issue run-messages <task-id>    # 查看某次执行的消息
```

#### Agent / Skill / Autopilot / Project / Repo

```bash
multimira agent list | get | create | update | archive
multimira skill list | get | create | update | delete | import | files upsert
multimira autopilot list | get | create | update | trigger
multimira autopilot trigger-add --cron "0 9 * * 1-5"
multimira project list | get | create | update
multimira repo list | add | update | delete
```

#### Runtime

```bash
multimira runtime list | usage | activity | update
```

#### 配置 / 更新

```bash
multimira config show | set server_url ...
multimira auth status | logout
multimira version | update
```

#### 产品里的位置

CLI 是 Multimira 对开发者友好度的体现。对于 agent 自己来说，也同等重要——**agent 在执行任务时能调用 `multimira` 命令读写 issue、评论、查文档**，这正是 CLI 在 "agent 作为一等公民"架构里的作用。
...
```
┌─────────────────────┐        ┌────────────────────┐        ┌──────────────────┐
│  Next.js Web App    │        │  Electron Desktop  │        │  multimira CLI     │
...
| **Agent CLI（Claude Code 等）** | 实际调用 LLM、执行工具调用、写文件、跑测试 | 不感知 Multimira 的数据模型（所有上下文通过 `multimira` CLI 命令读回） |
...
**Multimira 本身不直接调 LLM API**。所有 LLM 调用都在 agent CLI 子进程里发生（Claude Code 调 Anthropic API、Codex 调 OpenAI API 等）。
...
- **Invite accept overlay**（来自 `multimira://invite/{id}` 深链接）
...
- **深链接**：`multimira://auth/callback?token=...` 和 `multimira://invite/{id}`
...
Multimira 的设计可以归结为一句话：**把"人在一个看板上协作"这件事，扩展到了"人 + AI agent 在同一个看板上协作"**。
