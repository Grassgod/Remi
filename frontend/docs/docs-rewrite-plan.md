...
- `apps/docs/app/global.css` 里 `@import "@multiremi/ui/styles/tokens.css"`，覆盖 Fumadocs 的 `neutral.css`
...
当前 `.github/workflows/ci.yml:33` 用 `--filter='!@multiremi/docs'` 排除了 docs build。**在 Phase 0 做一个独立小 PR 把它加回来**——否则 MDX 语法错 CI 不拦，只有 Vercel 部署时才发现。
