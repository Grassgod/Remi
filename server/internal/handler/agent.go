...
type AgentTaskResponse struct {
	...
	ChatMessageAttachments  []ChatAttachmentMeta `json:"chat_message_attachments,omitempty"`  // attachments on the user message — agent calls `multimira attachment download <id>` per entry
	...
	// MULTIMIRA_TOKEN in the agent process environment. The server binds it to
...
// `multimira attachment download <id>` rather than guessing from the
...
func (h *Handler) UpdateAgent(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := rawFields["custom_env"]; ok {
		writeError(w, http.StatusBadRequest, "custom_env is no longer accepted on this endpoint; use PUT /api/agents/{id}/env (or `multimira agent env set`)")
		return
	}
