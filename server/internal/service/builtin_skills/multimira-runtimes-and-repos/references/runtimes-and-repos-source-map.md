...
- `server/cmd/multimira/cmd_runtime.go` registers `runtime list`, `usage`, `activity`, and `update`.
...
- `server/cmd/multimira/cmd_repo.go` registers `repo checkout <url> [--ref]`.
- `repo checkout` requires `MULTIMIRA_DAEMON_PORT`, sends `workspace_id`, `workdir`, `ref`, `agent_name`, and `task_id` to local daemon `/repo/checkout`, then prints the checked-out path.
