...
```bash
multimira agent get <agent-id> --output json      # full persisted agent record
multimira agent skills list <agent-id> --output json   # current skill bindings
multimira agent env get <agent-id> --output json  # plaintext env (owner/admin only, agents denied)
```
...
`POST /api/agents` (`multimira agent create`). At task claim time the daemon
...
```bash
multimira agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
multimira agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`multimira agent env get`). It is gated to workspace **owner/admin**
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`multimira agent env set`), which is owner/admin-only
...
```bash
multimira agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
multimira agent update <agent-id> --mcp-config-stdin --output json
multimira agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `multimira agent create` — inserts a new agent row.
- `multimira agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `multimira agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
