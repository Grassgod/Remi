...
Evidence layer for `multimira-skill-importing`. Every behavioral claim in `SKILL.md`
...
grep -n "func runSkillImport"           server/cmd/multimira/cmd_skill.go
...
## CLI: `multimira skill import --url`

| Behavior | File:line |
|---|---|
| `skill import` command def | `server/cmd/multimira/cmd_skill.go:60-64` |
| `--url` flag | `server/cmd/multimira/cmd_skill.go:143` |
| `--output` flag (default `json`) | `server/cmd/multimira/cmd_skill.go:144` |
| `runSkillImport` | `server/cmd/multimira/cmd_skill.go:412` |
| Requires `--url` | `server/cmd/multimira/cmd_skill.go:418-421` |
| `POST /api/skills/import` | `server/cmd/multimira/cmd_skill.go:431` |
| On error, tries conflict handler; returns `nil` (exit 0) when handled | `server/cmd/multimira/cmd_skill.go:432-434` |
...
| CLI `handleSkillImportConflict` | `server/cmd/multimira/cmd_skill.go:447` |
| Requires HTTP 409 + non-empty body | `server/cmd/multimira/cmd_skill.go:449-451` |
| Requires `existing_skill` key (else `false` → non-zero exit) | `server/cmd/multimira/cmd_skill.go:457-459` |
| Under `--output json`: prints body, returns `true` (caller exits 0) | `server/cmd/multimira/cmd_skill.go:461-465` |
...
| CLI `agent skills add` def ("without replacing existing assignments") | `server/cmd/multimira/cmd_agent.go:125-130` |
| `runAgentSkillsAdd` → `POST .../skills/add` | `server/cmd/multimira/cmd_agent.go:839`; POST `:860` |
| CLI `agent skills set` def ("replaces all current assignments") | `server/cmd/multimira/cmd_agent.go:118-123` |
| `runAgentSkillsSet` → `PUT .../skills` | `server/cmd/multimira/cmd_agent.go:814`; PUT `:832` |
| CLI `agent skills list` | `server/cmd/multimira/cmd_agent.go:782`; GET `:792` |
