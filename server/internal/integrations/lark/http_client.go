...
const (
	...
	// Operators do NOT set MULTIMIRA_LARK_HTTP_BASE_URL to pick a cloud
	...
	// latency from a self-hosted Multimira deployment to feishu.cn.
...
func (c HTTPClientConfig) withDefaults() HTTPClientConfig {
	...
	// non-empty BaseURL (MULTIMIRA_LARK_HTTP_BASE_URL, or an httptest URL
...
// keyed by app_id so a single Multimira server reuses Lark's
...
// cfg.BaseURL (MULTIMIRA_LARK_HTTP_BASE_URL, or an httptest URL in tests)
...
// SendMarkdownDM posts a schema-2.0 markdown card directly to a user's p2p
// chat (receive_id_type=open_id). Mirrors SendMarkdownCard but targets a user
// instead of a group chat — the path for proactive notifications (e.g. "your
// agent finished MUL-123"). Returns the card's message_id.
func (c *httpAPIClient) SendMarkdownDM(ctx context.Context, creds InstallationCredentials, openID, markdown, summary string) (string, error) {
	if openID == "" {
	...
	if summary != "" {
		card["config"] = map[string]any{"summary": map[string]any{"content": summary}}
	}
	cardBytes, err := json.Marshal(card)
	if err != nil {
		return "", fmt.Errorf("lark http client: encode markdown dm: %w", err)
	}
	q := url.Values{}
	...
	if err := c.doJSON(ctx, c.resolveBaseURL(creds), http.MethodPost, path, token, body, &resp); err != nil {
		return "", fmt.Errorf("lark http client: send markdown dm: %w", err)
	}
	if resp.Code != 0 || resp.Data.MessageID == "" {
		if isTokenError(resp.Code) {
			c.invalidateToken(creds.AppID)
		}
		return "", fmt.Errorf("lark http client: send markdown dm: code=%d msg=%q", resp.Code, resp.Msg)
	}
	return resp.Data.MessageID, nil
}
...
func bindingPromptTemplate(bindURL string) (string, error) {
	doc := map[string]any{
		...
		"header": map[string]any{
			"template": "blue",
			"title":    map[string]any{"tag": "plain_text", "content": "Multimira"},
		},
		"elements": []any{
			map[string]any{
				"tag": "div",
				"text": map[string]any{
					"tag":     "lark_md",
					"content": "你还没有绑定 Multimira 账户。点击下方按钮完成绑定后即可使用此 Agent。",
				},
			},
			map[string]any{
				"tag": "action",
