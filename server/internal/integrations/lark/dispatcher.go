...
type DispatchResult struct {
	...
	// even when the Multimira deep link is not reachable.
	IssueTitle string
}
...
func (d *Dispatcher) processClaimed(ctx context.Context, msg InboundMessage, inst db.LarkInstallation, claimToken pgtype.UUID) (DispatchResult, dedupFinalize, error) {
	...
	//    so we use them.
	sessionCreator := binding.MultimiraUserID
	...
	appendRes, err := d.Chat.AppendUserMessage(ctx, AppendUserMessageParams{
		...
		Sender:         binding.MultimiraUserID,
		Body:           msg.Body,
	...
	if appendRes.IssueCommand != nil {
		issueRes, err := d.createIssueFromCommand(ctx, inst, binding.MultimiraUserID, sessionID, *appendRes.IssueCommand)
		if err != nil {
