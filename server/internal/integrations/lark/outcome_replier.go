...
type LarkOutcomeReplier struct {
	...
	publicURL    string // e.g. https://multimira.example, trailing slash trimmed
...
// Multimira HTTP host the user clicks into to redeem the binding token
// (e.g. https://multimira.example); empty means the binding flow can
...
func NewLarkOutcomeReplier(cfg OutcomeReplierConfig) OutcomeReplier {
	...
	if cfg.PublicURL == "" {
		log.Warn("lark outcome replier: MULTIMIRA_PUBLIC_URL not set; binding prompt CTA will not work")
	}
...
// after MUL-2671's plain-text refactor. The link to Multimira is
...
// MULTIMIRA_PUBLIC_URL) the message still confirms the issue, just
