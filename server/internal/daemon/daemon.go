...
import (
	...
	"github.com/multimira-ai/multimira/server/internal/daemon/repocache"
	"github.com/multimira-ai/multimira/server/pkg/agent"
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'multimira login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'multimira login --profile %s'", d.cfg.Profile)
		}
...
// `multimira repo checkout` accepts project-only URLs without an extra round
...
// other API call is what surfaces a user-actionable "run multimira login"
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Multimira Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/multimira)
...
func (d *Daemon) triggerRestart() {
	...
	// must use the stable <brew-prefix>/bin/multimira symlink instead.
	if isBrewInstall() {
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			newBin = filepath.Join(brewPrefix, "bin", "multimira")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			newBin = filepath.Join(prefix, "bin", "multimira")
		} else {
...
// release. Used by pollLoop to expose MULTIMIRA_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (TaskResult, error) {
	...
	// here would make MULTIMIRA_WORKSPACE_ID empty in the agent env, and the
	...
	// `multimira repo checkout` would reject project-only URLs that aren't also
	...
	// via `multimira repo checkout <url>`.
	...
	// up stale Multimira instructions (issue id, trigger comment id, reply
	...
	if env.LocalDirectory {
		defer func() {
			...
			// Excise the sidecar tree (.agent_context/, .multimira/,
	...
	// can call the Multimira API and the local daemon (e.g. `multimira repo checkout`).
	// MULTIMIRA_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	// MULTIMIRA_TOKEN is the credential the agent process will use to call the
	// Multimira API. Prefer the task-scoped token the server minted at claim
	...
	agentEnv := map[string]string{
		"MULTIMIRA_TOKEN":        agentToken,
		"MULTIMIRA_SERVER_URL":   d.cfg.ServerBaseURL,
		"MULTIMIRA_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"MULTIMIRA_WORKSPACE_ID": task.WorkspaceID,
		"MULTIMIRA_AGENT_NAME":   agentName,
		"MULTIMIRA_AGENT_ID":     task.AgentID,
		"MULTIMIRA_TASK_ID":      task.ID,
		"MULTIMIRA_TASK_SLOT":    strconv.Itoa(slot),
	}
	if task.AutopilotRunID != "" {
		agentEnv["MULTIMIRA_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["MULTIMIRA_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the multimira CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["MULTIMIRA_QUICK_CREATE_TASK_ID"] = task.ID
	}
	// Ensure the multimira CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// multimira binary so that `multimira` commands in the agent always resolve.
	...
	// then the daemon-wide MULTIMIRA_<PROVIDER>_MODEL env var. If
	...
	// `multimira issue status` / `multimira issue comment add`, leaving issues
	// stuck in `todo`.
	//
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "MULTIMIRA_") {
		return true
	}
	switch upper {
