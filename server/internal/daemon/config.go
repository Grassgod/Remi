...
const (
	...
	// hard ceiling for cost/resource control can set MULTIMIRA_AGENT_TIMEOUT.
	...
	// Set MULTIMIRA_AGENT_IDLE_WATCHDOG=0 to disable.
	...
	// that stuck-tool case (MUL-3064). Set MULTIMIRA_AGENT_TOOL_WATCHDOG=0 to
...
// MULTIMIRA_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // multimira CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/multimira_workspaces)
	...
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on Multimira Cloud, false on self-host)
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("MULTIMIRA_SERVER_URL", DefaultServerURL)
	...
	// pinned to an explicit MULTIMIRA_*_PATH) must not pay the cost of
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned MULTIMIRA_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("MULTIMIRA_CLAUDE_PATH", "claude", "MULTIMIRA_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("MULTIMIRA_CODEX_PATH", "codex", "MULTIMIRA_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("MULTIMIRA_OPENCODE_PATH", "opencode", "MULTIMIRA_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("MULTIMIRA_OPENCLAW_PATH", "openclaw", "MULTIMIRA_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("MULTIMIRA_HERMES_PATH", "hermes", "MULTIMIRA_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("MULTIMIRA_GEMINI_PATH", "gemini", "MULTIMIRA_GEMINI_MODEL"); ok {
		agents["gemini"] = e
	}
	if e, ok := probe("MULTIMIRA_PI_PATH", "pi", "MULTIMIRA_PI_MODEL"); ok {
		agents["pi"] = e
	}
	if e, ok := probe("MULTIMIRA_CURSOR_PATH", "cursor-agent", "MULTIMIRA_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("MULTIMIRA_COPILOT_PATH", "copilot", "MULTIMIRA_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("MULTIMIRA_KIMI_PATH", "kimi", "MULTIMIRA_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("MULTIMIRA_KIRO_PATH", "kiro-cli", "MULTIMIRA_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	...
	if e, ok := probe("MULTIMIRA_ANTIGRAVITY_PATH", "agy", ""); ok {
	...
	claudeArgs, err := shellArgsFromEnv("MULTIMIRA_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("MULTIMIRA_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("MULTIMIRA_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("MULTIMIRA_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("MULTIMIRA_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("MULTIMIRA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// MULTIMIRA_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("MULTIMIRA_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// MULTIMIRA_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("MULTIMIRA_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("MULTIMIRA_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via MULTIMIRA_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("MULTIMIRA_DAEMON_ID"))
	...
	// — e.g. ~/.multimira/daemon.id (default) plus ~/.multimira/profiles/<x>/
	...
	// the user explicitly pins MULTIMIRA_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("MULTIMIRA_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("MULTIMIRA_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/multimira_workspaces or ~/multimira_workspaces_<profile>)
	...
	keepEnv := os.Getenv("MULTIMIRA_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("MULTIMIRA_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("MULTIMIRA_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("MULTIMIRA_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("MULTIMIRA_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("MULTIMIRA_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("MULTIMIRA_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("MULTIMIRA_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on Multimira Cloud (api.multimira.ai) and opt-out for
	...
	// Operators on either side can flip the default with MULTIMIRA_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := isOfficialCloudServer(serverBaseURL)
	if v := strings.TrimSpace(os.Getenv("MULTIMIRA_DAEMON_AUTO_UPDATE")); v != "" {
	...
	autoUpdateInterval, err := durationFromEnv("MULTIMIRA_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
...
// officialCloudHost is the hostname of Multimira's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.multimira.ai subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.multimira.ai"

// isOfficialCloudServer reports whether the resolved server base URL points
// at Multimira's hosted cloud. Used to pick the auto-update default: cloud
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid MULTIMIRA_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("MULTIMIRA_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
// MULTIMIRA_WORKSPACES_ROOT env > default ($HOME/multimira_workspaces, or
// $HOME/multimira_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `multimira daemon disk-usage`) use this directly so they pick the same
// directory the running daemon would have picked.
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("MULTIMIRA_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set MULTIMIRA_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "multimira_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "multimira_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("MULTIMIRA_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// before any MULTIMIRA_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom MULTIMIRA_*_PATH values never reach this
