...
type Task struct {
	...
	ChatMessageAttachments  []ChatAttachmentMeta  `json:"chat_message_attachments,omitempty"`  // attachments linked to the chat message; agent uses these to `multimira attachment download <id>`
	...
	// The daemon injects it into the spawned agent as MULTIMIRA_TOKEN so the
...
// run `multimira attachment download <id>` instead of guessing from a
