...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/multimira-ai/multimira","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".multimira", "project", "resources.json")
	...
	for _, want := range []string{
		...
		"Agent UX 2026",
		"GitHub repo",
		"https://github.com/multimira-ai/multimira",
		"default branch: `main`",
		".multimira/project/resources.json",
	} {
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".multimira", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"multimira autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `multimira issue get") {
...
// collision-free duplicates (issue-review, issue-review-multimira,
// issue-review-multimira-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -multimira duplicates", names)
	}
...
// next refresh dodges to issue-review-multimira. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -multimira duplicate", names)
	}
...
func TestInjectRuntimeConfigClaude(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"Multimira Agent Runtime",
		"multimira issue get",
		"multimira issue comment list",
		"Go Conventions",
		"PR Review",
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"`multimira <command> --help`",
		"multimira issue get <id> --output json",
		"multimira issue comment list <issue-id>",
		"multimira issue create --title",
		"multimira issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"multimira repo checkout <url>",
		"multimira issue status <id> <status>",
		"multimira issue comment add <issue-id>",
		"multimira issue comment add --help",
		"multimira squad member set-role <squad-id>",
	} {
		if !strings.Contains(s, want) {
	...
	for _, banned := range []string{
		"multimira issue list [--status",
		"multimira issue label list",
		"multimira issue subscriber list",
		"multimira label list",
		"multimira workspace member list",
		"multimira agent list",
		"multimira squad list",
		"multimira issue runs",
		"multimira issue run-messages",
		"multimira attachment download",
		"multimira autopilot list",
		"multimira autopilot create",
		"multimira autopilot update",
		"multimira autopilot trigger",
		"multimira autopilot delete",
		"multimira project get",
		"multimira project resource list",
		"multimira issue assign",
		"multimira issue label add",
		"multimira issue label remove",
		"multimira issue subscriber add",
		"multimira issue subscriber remove",
		"multimira issue comment delete",
		"multimira label create",
	} {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Multimira Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "multimira issue get") {
		t.Error("should reference multimira CLI even without skills")
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `multimira issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"multimira issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `multimira issue comment add`",
...
// non-ASCII as `?` before reaching `multimira.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
`
	...
	for _, marker := range []string{
		multimiraManagedBeginMarker,
		multimiraMultiAgentBeginMarker,
		multimiraMemoryFeatureBeginMarker,
		multimiraMemoryConfigBeginMarker,
	} {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, multimiraManagedBeginMarker) || !strings.Contains(s, multimiraManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertMultimiraManagedBlock
	// for why.
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), multimiraManagedBeginMarker); n != 1 {
...
`
	...
	beginIdx := strings.Index(s, multimiraManagedBeginMarker)
	endIdx := strings.Index(s, multimiraManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.multimira]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.multimira]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.multimira content")
	}
	...
	if n := strings.Count(string(data2), multimiraManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.multimira], got:\n%s", s)
	}
	if strings.Count(s, multimiraManagedBeginMarker) != 1 {
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), multimiraManagedBeginMarker) {
		t.Error("reused config.toml missing multimira-managed block")
	}
}
...
// inside a Multimira task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigCommentTriggerColdStartRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"multimira issue comment list " + issueID + " --thread " + triggerID + " --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "multimira issue comment list "+issueID+" --thread "+triggerID+" --output json") {
...
func TestInjectRuntimeConfigCommentTriggerResumedNoDeltaRead(t *testing.T) {
	...
	for _, want := range []string{
		...
		"multimira issue comment list " + issueID + " --thread thread-root-1 --tail 30 --output json",
	} {
		if !strings.Contains(s, want) {
...
func TestInjectRuntimeConfigAssignmentTriggerMentionsRecent(t *testing.T) {
	...
	for _, want := range []string{
		"multimira issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"multimira issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"multimira issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `multimira issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"multimira issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"multimira issue metadata set",
				"multimira issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_heredoc", func(t *testing.T) {
		...
		if !strings.Contains(s, "multimira issue metadata list issue-md-codex --output json") {
