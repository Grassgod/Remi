...
// nothing related to Multimira before a task ran must contain nothing
// related to Multimira after the task finishes — no .agent_context/,
// no .claude/skills/, no .multimira/, no stub directories. The test
...
// must still exist and the Multimira-written sibling must be gone.
...
// round-trip to the .multimira/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .multimira/sidecar.txt and created
	// .multimira/ + .multimira/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .multimira/.
	managedDir := filepath.Join(workDir, ".multimira")
	...
	// .multimira still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-multimira/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Multimira would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Multimira skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Multimira
// sibling (which lives at `<slug>-multimira`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Multimira's version",
						Content:     "---\nname: issue-review\n---\n\nMultimira skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Multimira-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.multimira/project/
// resources.json` — the other Multimira-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".multimira", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".multimira", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-multimira`, then `-multimira-2`,
// `-multimira-3`, … The PR-review concern is "Multimira skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-multimira`.
	...
	if slug != "issue-review-multimira" {
		t.Errorf("second allocation should bump to `-multimira`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-multimira") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-multimira"), 0o755); err != nil {
	...
	if slug != "issue-review-multimira-2" {
		t.Errorf("third allocation should be `-multimira-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-multimira-2") {
...
// `Issue Review` skill, the Multimira sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Multimira sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Multimira skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Multimira's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	multimiraDir := filepath.Join(workDir, ".claude", "skills", "issue-review-multimira")
	if _, err := os.Stat(filepath.Join(multimiraDir, "SKILL.md")); err != nil {
		t.Errorf("Multimira sibling skill should exist at %s: %v", multimiraDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Multimira sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(multimiraDir); !os.IsNotExist(err) {
		t.Errorf("Multimira sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
