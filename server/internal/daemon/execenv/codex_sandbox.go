...
// `no such host` errors when calling out (for example, `multimira issue get`
// hitting the Multimira API). See upstream issue openai/codex#10390.
//
...
// actually reach the Multimira API. On Linux (and on macOS once the upstream
...
//   danger-full-access so the Multimira CLI can reach the API.
...
// multimiraManagedBeginMarker / multimiraManagedEndMarker delimit the block the
...
const (
	multimiraManagedBeginMarker = "# BEGIN multimira-managed (do not edit; regenerated by daemon)"
	multimiraManagedEndMarker   = "# END multimira-managed"
)

// renderMultimiraManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.multimira]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertMultimiraManagedBlock), avoids both traps.
func renderMultimiraManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(multimiraManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(multimiraManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(multimiraManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(multimiraManagedEndMarker) + `\n*`)

// upsertMultimiraManagedBlock returns the config content with the multimira-managed
...
// `[permissions.multimira]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderMultimiraManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertMultimiraManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderMultimiraManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the multimira-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertMultimiraManagedBlock(existing, policy)
