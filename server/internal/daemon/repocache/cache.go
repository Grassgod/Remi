...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/multimira_workspaces/.repos)
	logger *slog.Logger
...
// multimiraHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const multimiraHookMarker = "# multimira:prepare-commit-msg:co-authored-by"
...
// treats a hook as Multimira-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before multimiraHookMarker existed, so disabling the toggle on
...
// Co-authored-by trailer for the Multimira Agent to every commit message.
...
// a Co-authored-by trailer for the Multimira Agent. The hook is installed in the
...
// installed by the Multimira daemon (current or any previously released
