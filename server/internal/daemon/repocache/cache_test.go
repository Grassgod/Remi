...
// for the Multimira Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: multimira-agent <github@multimira.ai>"
...
func TestCoAuthoredByHookIdempotent(t *testing.T) {
	...
	trailer := "Co-authored-by: multimira-agent <github@multimira.ai>"
	if err := os.WriteFile(filepath.Join(result.Path, "test.txt"), []byte("hello\n"), 0o644); err != nil {
		t.Fatalf("write test file: %v", err)
	}
	runGitAuthored(t, result.Path, "add", ".")
...
// path: a bare cache that already carries the Multimira prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: multimira-agent") {
...
// prepare-commit-msg hook that does NOT include the multimiraHookMarker
// sentinel — only the older `# Installed by the Multimira daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no multimiraHookMarker line). Keeping a
...
# Multimira: add Co-authored-by trailer for the Multimira Agent.
# Installed by the Multimira daemon. Do not edit — it will be overwritten.
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: multimira-agent") {
...
// without the Multimira marker (e.g. one a user added manually) must be left
