...
// same machine shares one UUID stored at `~/.multimira/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.multimira/profiles/*/daemon.id` and returns every
...
// explicitly pins MULTIMIRA_DAEMON_ID to the hostname itself, there's nothing
