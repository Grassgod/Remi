...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers MULTIMIRA_APP_URL", func(t *testing.T) {
		t.Setenv("MULTIMIRA_APP_URL", "http://localhost:14000")
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `multimira login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}
...
// `multimira login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Multimira Cloud Node PATs (`mcn_`) at
