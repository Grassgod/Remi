...
// agent task (MULTIMIRA_AGENT_ID / MULTIMIRA_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.multimira/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("outside agent context falls back to config", func(t *testing.T) {
		t.Setenv("MULTIMIRA_AGENT_ID", "")
		t.Setenv("MULTIMIRA_TASK_ID", "")
		t.Setenv("MULTIMIRA_WORKSPACE_ID", "")
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("MULTIMIRA_AGENT_ID", "agent-123")
		t.Setenv("MULTIMIRA_TASK_ID", "task-456")
		t.Setenv("MULTIMIRA_WORKSPACE_ID", "env-ws")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("MULTIMIRA_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("MULTIMIRA_WORKSPACE_ID", "test-ws")
	...
	msg := err.Error()
	if !strings.Contains(msg, "multimira agent env set") {
		t.Fatalf("no-fields error must direct users to `multimira agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `multimira agent env set` instead", flag)
		}
	}
}
