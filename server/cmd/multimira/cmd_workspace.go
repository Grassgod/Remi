...
var workspaceSwitchCmd = &cobra.Command{
	...
	Long: "Sets the default workspace for the current profile after verifying you " +
		...
		"commands without --workspace-id or MULTIMIRA_WORKSPACE_ID will target " +
		"this workspace.\n\n" +
		"Resolution priority (highest to lowest): --workspace-id flag, " +
		"MULTIMIRA_WORKSPACE_ID env, profile default (set by this command).\n\n" +
		"For low-level use, 'multimira config set workspace_id <id>' writes the " +
...
func fetchWorkspaces(ctx context.Context, cmd *cobra.Command) ([]workspaceSummary, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("not authenticated: run 'multimira login' first")
	}
...
func runWorkspaceList(cmd *cobra.Command, _ []string) error {
	...
	if currentID != "" {
		fmt.Fprintln(os.Stderr, "\n* = current default workspace (use 'multimira workspace switch <id|slug|prefix>' to change)")
	} else {
		fmt.Fprintln(os.Stderr, "\nNo default workspace set. Use 'multimira workspace switch <id|slug|prefix>' to pick one.")
	}
...
func resolveWorkspaceByIDOrSlug(workspaces []workspaceSummary, target string) (workspaceSummary, error) {
	...
	return workspaceSummary{}, fmt.Errorf("workspace %q not found or you do not have access; run 'multimira workspace list' to see options", target)
}
...
func runWorkspaceGet(cmd *cobra.Command, args []string) error {
	...
	if wsID == "" {
		return fmt.Errorf("workspace ID is required: pass an id/slug/prefix as argument or set MULTIMIRA_WORKSPACE_ID")
	}
