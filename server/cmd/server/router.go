...
import (
	...
	"github.com/multimira-ai/multimira/server/internal/daemonws"
	"github.com/multimira-ai/multimira/server/internal/events"
	"github.com/multimira-ai/multimira/server/internal/handler"
	"github.com/multimira-ai/multimira/server/internal/integrations/lark"
	obsmetrics "github.com/multimira-ai/multimira/server/internal/metrics"
	"github.com/multimira-ai/multimira/server/internal/middleware"
	"github.com/multimira-ai/multimira/server/internal/realtime"
	"github.com/multimira-ai/multimira/server/internal/service"
	"github.com/multimira-ai/multimira/server/internal/storage"
	"github.com/multimira-ai/multimira/server/internal/util"
	"github.com/multimira-ai/multimira/server/internal/util/secretbox"
...
// MULTIMIRA_TRUSTED_PROXIES env var. Invalid entries are dropped with a single
...
func parseTrustedProxies(raw string) []netip.Prefix {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if err != nil {
			slog.Warn("MULTIMIRA_TRUSTED_PROXIES: ignoring invalid CIDR",
				"value", s, "error", err)
			continue
		}
		out = append(out, p)
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	signupConfig := handler.Config{
		...
		PublicURL:                strings.TrimRight(strings.TrimSpace(os.Getenv("MULTIMIRA_PUBLIC_URL")), "/"),
		TrustedProxies:           parseTrustedProxies(os.Getenv("MULTIMIRA_TRUSTED_PROXIES")),
	...
	// Lark integration. Only wired when MULTIMIRA_LARK_SECRET_KEY is set:
	...
	if larkKey, err := secretbox.LoadKey("MULTIMIRA_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// MULTIMIRA_LARK_SECRET_KEY is the operator's opt-in for
				...
				// real Lark traffic can point MULTIMIRA_LARK_HTTP_BASE_URL
				// at a mock server.
				//
				// MULTIMIRA_LARK_HTTP_BASE_URL is an OPTIONAL deployment-wide
				...
				larkClient := lark.NewHTTPAPIClient(lark.HTTPClientConfig{
					BaseURL: strings.TrimSpace(os.Getenv("MULTIMIRA_LARK_HTTP_BASE_URL")),
					Logger:  slog.Default(),
				})
				...
				// Proactive Feishu DM push: the real HTTP client supports
				// direct messages (the stub does not), so wire TaskService to
				// send a best-effort DM card to a member after an inbox
				// notification is created. Nil-safe: if the client can't DM,
				// the notifier is simply never set.
				if sender, ok := lark.APIClient(larkClient).(lark.DMSender); ok {
					notifier := lark.NewUserNotifier(pool, queries, installSvc.DecryptAppSecret, sender, slog.Default())
					h.TaskService.DMNotifier = notifier.NotifyUser
				}
				...
				// to initialize (bad MULTIMIRA_LARK_CALLBACK_BASE_URL or
				...
				go lark.BackfillRegionFromLegacyOverride(context.Background(), queries,
					strings.TrimSpace(os.Getenv("MULTIMIRA_LARK_HTTP_BASE_URL")),
					strings.TrimSpace(os.Getenv("MULTIMIRA_LARK_CALLBACK_BASE_URL")),
					slog.Default())
				...
				// MULTIMIRA_LARK_REGISTRATION_DOMAIN / _LARK_DOMAIN env
				// vars override the protocol hosts for staging / dev.
				regCfg := lark.RegistrationConfig{
					Domain:     strings.TrimSpace(os.Getenv("MULTIMIRA_LARK_REGISTRATION_DOMAIN")),
					LarkDomain: strings.TrimSpace(os.Getenv("MULTIMIRA_LARK_REGISTRATION_LARK_DOMAIN")),
				}
		...
		slog.Info("lark integration disabled (MULTIMIRA_LARK_SECRET_KEY not set)")
	}
	...
	// Cloud PAT verifier: validates mcn_ tokens against Multimira Cloud
	...
	// Reads MULTIMIRA_CLOUD_FLEET_URL / MULTIMIRA_FLEET_URL directly so a
	// deployment opting in to cloud PATs doesn't need a second config knob.
	cloudPATVerifier := auth.NewCloudPATVerifier(auth.CloudPATVerifierConfig{
		FleetBaseURL: cloudRuntimeFleetURLFromEnv(),
		Redis:        rdb,
	})
	...
	// Feishu (Lark) SSO — the primary login for the self-hosted internal build.
	r.With(authRL).Get("/auth/lark/url", h.LarkLoginURL)
	r.With(authVerifyRL).Post("/auth/lark/callback", h.LarkLoginCallback)
	...
	// GitHub App webhook (no Multimira auth — requests are authenticated via
...
// MULTIMIRA_LARK_CALLBACK_BASE_URL), we log and fall back to the
...
func buildLarkConnectorFactory(installSvc *lark.InstallationService, apiClient lark.APIClient) (lark.ConnectorFactory, string) {
	endpointFetcher, err := lark.NewHTTPConnectionTokenFetcher(lark.HTTPConnectionTokenConfig{
		BaseURL: strings.TrimSpace(os.Getenv("MULTIMIRA_LARK_CALLBACK_BASE_URL")),
		Logger:  slog.Default(),
	})
	if err != nil {
...
func cloudRuntimeFleetURLFromEnv() string {
	if url := strings.TrimSpace(os.Getenv("MULTIMIRA_CLOUD_FLEET_URL")); url != "" {
		return url
	}
	return strings.TrimSpace(os.Getenv("MULTIMIRA_FLEET_URL"))
}
