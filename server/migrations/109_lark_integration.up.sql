...
-- lark_installation
-- =====================
-- One row per (workspace, agent) — each Multimira Agent owns at most one
...
-- lark_user_binding
-- =====================
-- Maps a Lark `open_id` to a Multimira user, per-installation. open_id is
...
--
--   2. The composite FK on (workspace_id, multimira_user_id) targets
--      member(workspace_id, user_id) with ON DELETE CASCADE, so when a
--      Multimira user is removed from the workspace the stale Lark
...
CREATE TABLE lark_user_binding (
    ...
    multimira_user_id  UUID NOT NULL,
...
-- =====================
-- One Lark chat (`chat_id`) ↔ one Multimira `chat_session`. The Lark side
...
-- =====================
-- Maps a Multimira task (or session bootstrap "thinking…" card) to the
...
-- click here" flow that links a Lark `open_id` to a Multimira user. The
