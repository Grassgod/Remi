...
// the multimira CLI version reported by the daemon at registration time. The
...
// `multimira issue create` failure — see PR #1851); older daemons would either
...
var (
	ErrCLIVersionMissing = errors.New("multimira CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("multimira CLI version is below required minimum")
)
