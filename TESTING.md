...
| `.github/workflows/release.yml` | push tag `v*` | 校验依赖准备快照、tag 版本与 main 提交全量 CI 成功 → `bun run build:multiremi` → 上传 tar.gz + `scripts/install-remi.sh` 到 GitHub Release(release notes 由 `generate_release_notes` 自动生成) |
