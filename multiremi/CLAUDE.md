...
Multimira is an AI-native task management platform — like Linear, but with AI agents as first-class citizens.
...
- **Mobile (`apps/mobile/`) is independent.** It shares only **types and pure functions** from `@multimira/core/`, with `import type` for types (zero runtime coupling). UI, state, hooks, providers, i18n, React version, build pipeline, release cadence — all mobile-owned.
...
make cli ARGS="..."   # Run multimira CLI (e.g. make cli ARGS="config")
make test             # Go tests
...
pnpm --filter @multimira/views exec vitest run auth/login-page.test.tsx
pnpm --filter @multimira/core exec vitest run runtimes/version.test.ts
pnpm --filter @multimira/web exec vitest run app/\(auth\)/login/page.test.tsx
...
pnpm --filter @multimira/desktop build      # Compile TS → JS (reads .env.production)
pnpm --filter @multimira/desktop package    # Package into .app/.dmg/.exe (current platform only)
...
- `packages/ui/` — zero `@multimira/core` imports (pure UI, no business logic).
...
Rules for `apps/mobile/` live in `apps/mobile/CLAUDE.md`. Read it before touching anything in `apps/mobile/` — it covers what may be imported from `@multimira/core/`, the React version policy, the build/release pipeline, and the locked tech-stack baseline.
...
Every full-window desktop view (anything outside the dashboard shell) must mount `<DragStrip />` from `@multimira/views/platform` as the first flex child of the page root, otherwise users can't drag the window. Interactive UI inside the top 48px needs `WebkitAppRegion: "no-drag"` to stay clickable.
...
**Never test shared component behavior in an app's test file.** If a test requires mocking `next/navigation` or `react-router-dom` to test a component from `@multimira/views`, the test is in the wrong place — move it to `packages/views/` and mock `@multimira/core` instead.
...
- Mock `@multimira/core` stores with `vi.hoisted()` + `Object.assign(selectorFn, { getState })` pattern (Zustand stores are both callable and have `.getState()`).
- Mock `@multimira/core/api` for API calls.
