#!/usr/bin/env bash
# Multimira installer — installs the CLI and optionally provisions a self-host server.
#
...
#
# After installation, run `multimira setup` to configure your environment.
#
set -euo pipefail
...
# Configuration
# ---------------------------------------------------------------------------
REPO_URL="https://github.com/multimira-ai/multimira.git"
REPO_WEB_URL="https://github.com/multimira-ai/multimira"  # without .git, for GitHub web APIs
INSTALL_DIR="${MULTIMIRA_INSTALL_DIR:-$HOME/.multimira/server}"
BREW_PACKAGE="multimira-ai/tap/multimira"
...
detect_os() {
  ...
  irm https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.ps1 | iex" ;;
    *)      fail "Unsupported operating system: $(uname -s). Multimira supports macOS, Linux, and Windows." ;;
  esac
...
install_cli_brew() {
  info "Installing Multimira CLI via Homebrew..."
  local brew_log
  brew_log=$(mktemp)
  if ! brew tap multimira-ai/tap >"$brew_log" 2>&1; then
  ...
  if ! brew install "$BREW_PACKAGE" >"$brew_log" 2>&1; then
    if brew list "$BREW_PACKAGE" >/dev/null 2>&1; then
      rm -f "$brew_log"
      ok "Multimira CLI already installed via Homebrew"
    else
      warn "Failed to install multimira via Homebrew. Falling back to GitHub Releases binary install."
      _dump_brew_log "$brew_log"
      rm -f "$brew_log"
      return 1
    fi
    ...
    rm -f "$brew_log"
    ok "Multimira CLI installed via Homebrew"
  fi
}

install_cli_binary() {
  info "Installing Multimira CLI from GitHub Releases..."
  ...
  local url="https://github.com/multimira-ai/multimira/releases/download/${latest}/multimira-cli-${version}-${OS}-${ARCH}.tar.gz"
  local tmp_dir
  tmp_dir=$(mktemp -d)

  info "Downloading $url ..."
  if ! curl -fsSL "$url" -o "$tmp_dir/multimira.tar.gz"; then
    rm -rf "$tmp_dir"
    fail "Failed to download CLI binary."
  fi

  tar -xzf "$tmp_dir/multimira.tar.gz" -C "$tmp_dir" multimira

  # Try /usr/local/bin first, fall back to ~/.local/bin. Tests and scripted
  # installs can override the first choice with MULTIMIRA_BIN_DIR.
  local bin_dir="${MULTIMIRA_BIN_DIR:-/usr/local/bin}"
  if [ -w "$bin_dir" ]; then
    mv "$tmp_dir/multimira" "$bin_dir/multimira"
  elif command_exists sudo; then
    sudo mv "$tmp_dir/multimira" "$bin_dir/multimira"
  else
    bin_dir="$HOME/.local/bin"
    mkdir -p "$bin_dir"
    mv "$tmp_dir/multimira" "$bin_dir/multimira"
    chmod +x "$bin_dir/multimira"
  ...
  rm -rf "$tmp_dir"
  ok "Multimira CLI installed to $bin_dir/multimira"
}
...
get_selfhost_ref() {
  if [ -n "${MULTIMIRA_SELFHOST_REF:-}" ]; then
    printf '%s' "$MULTIMIRA_SELFHOST_REF"
    return
  fi
...
upgrade_cli_brew() {
  info "Upgrading Multimira CLI via Homebrew..."
  brew update 2>/dev/null || true
  if brew upgrade "$BREW_PACKAGE" 2>/dev/null; then
    ok "Multimira CLI upgraded via Homebrew"
  else
    # brew upgrade exits non-zero if already up to date
    ok "Multimira CLI is already the latest version"
  fi
}

install_cli() {
  if command_exists multimira; then
    local current_ver
    # `multimira version` outputs "multimira v0.1.13 (commit: abc1234)" — extract just the version
    current_ver=$(multimira version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ...
    if [ -z "$latest_ver" ] || [ "$current_cmp" = "$latest_cmp" ]; then
      ok "Multimira CLI is up to date ($current_ver)"
      return 0
    fi

    info "Multimira CLI $current_ver installed, latest is $latest_ver — upgrading..."
    ...
    local new_ver
    new_ver=$(multimira version 2>/dev/null | awk '{print $2}' || echo "unknown")
    ok "Multimira CLI upgraded ($current_ver → $new_ver)"
    return 0
  fi
  ...
  # Verify
  if ! command_exists multimira; then
    fail "CLI installed but 'multimira' not found on PATH. You may need to restart your shell."
  fi
}
...
check_docker() {
  if ! command_exists docker; then
    printf "\n"
    fail "Docker is not installed. Multimira self-hosting requires Docker and Docker Compose.
...
# ---------------------------------------------------------------------------
setup_server() {
  info "Setting up Multimira server..."
  local server_ref
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    info "Cloning Multimira repository..."
  ...
  info "Pulling official Multimira images..."
  pull_official_selfhost_images
  info "Starting Multimira services (this may take a few minutes on first run)..."
  ...
  if [ "$ready" = true ]; then
    ok "Multimira server is running"
  else
...
run_default() {
  printf "\n"
  printf "${BOLD}  Multimira — Installer${RESET}\n"
  printf "\n"
  ...
  printf "${BOLD}${GREEN}  ✓ Multimira CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  printf "  ${BOLD}Next: configure your environment${RESET}\n"
  printf "\n"
  printf "     ${CYAN}multimira setup${RESET}                # Connect to Multimira Cloud (multimira.ai)\n"
  printf "     ${CYAN}multimira setup self-host${RESET}       # Connect to a self-hosted server\n"
  printf "\n"
  printf "  ${BOLD}Self-hosting?${RESET} Install the server first:\n"
  printf "     curl -fsSL https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.sh | bash -s -- --with-server\n"
  printf "\n"
}
...
run_with_server() {
  printf "\n"
  printf "${BOLD}  Multimira — Self-Host Installer${RESET}\n"
  ...
  printf "\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "${BOLD}${GREEN}  ✓ Multimira server is running and CLI is ready!${RESET}\n"
  printf "${BOLD}${GREEN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RESET}\n"
  printf "\n"
  ...
  printf "\n"
  printf "     ${CYAN}multimira setup self-host${RESET}   # Configure + authenticate + start daemon\n"
  printf "\n"
  ...
  printf "     curl -fsSL https://raw.githubusercontent.com/multimira-ai/multimira/main/scripts/install.sh | bash -s -- --stop\n"
  printf "\n"
}
...
run_stop() {
  printf "\n"
  info "Stopping Multimira services..."
  ...
  if [ -d "$INSTALL_DIR" ]; then
    ...
    warn "No Multimira installation found at $INSTALL_DIR"
  fi

  if command_exists multimira; then
    multimira daemon stop 2>/dev/null && ok "Daemon stopped" || true
  fi
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --help|-h)
        ...
        echo ""
        echo "  (default)       Install / upgrade the Multimira CLI"
        echo "  --with-server   Install CLI + provision a self-host server (Docker)"
        echo "  --stop          Stop a self-hosted installation"
        echo ""
        echo "Environment variables:"
        echo "  MULTIMIRA_INSTALL_DIR   Self-host server install directory"
        echo "                        (default: \$HOME/.multimira/server)"
        echo "  MULTIMIRA_BIN_DIR       Target directory for the CLI binary when"
        echo "                        installing from GitHub Releases"
        echo "                        (default: /usr/local/bin, then \$HOME/.local/bin)"
        echo "  MULTIMIRA_SELFHOST_REF  Git ref to check out for self-host assets"
        echo "                        (default: latest release tag, falling back to main)"
        echo ""
        echo "After installation, run 'multimira setup' to configure your environment."
        exit 0
        ;;
