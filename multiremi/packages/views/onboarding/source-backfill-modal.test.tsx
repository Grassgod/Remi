...
vi.mock("@multimira/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@multimira/core/onboarding")>(
      "@multimira/core/onboarding",
    );
  return { ...actual, saveQuestionnaire: mockSaveQuestionnaire };
});

vi.mock("@multimira/core/analytics", () => ({
...
describe("SourceBackfillModal", () => {
  it("does not render when there is no user", () => {
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Multimira/i),
  ...
  it("opens for an onboarded user with empty source and fires the shown event", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Multimira/i),
      ).toBeInTheDocument();
    });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("multimira.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
