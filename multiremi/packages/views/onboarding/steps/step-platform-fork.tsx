...
import { captureEvent, setPersonProperties } from "@multimira/core/analytics";
...
import type { AgentRuntime } from "@multimira/core/types";
import { DragStrip } from "@multimira/views/platform";
...
/**
 * Step 3 on **web** (self-host). The user is in a browser, so we can't
 * scan their machine for runtimes. The single card opens a dialog with
 * the CLI install instructions + a live runtime probe: when the central
 * daemon registers and the user selects it, the dialog's "Connect &
 * continue" button fires `onNext(runtime)` and advances the flow.
 *
 * The desktop-download and cloud-computer cards were removed — this build
 * is web-only and agents run on a central CLI daemon.
 *
 * Footer has no Continue button (the CLI dialog owns advancement); only
 * Skip remains.
 */
        ...
        >
          <div className="mx-auto w-full max-w-[620px] px-6 py-10 sm:px-10 md:px-14 lg:px-0 lg:py-14">
            ...
            <div className="mt-10 flex max-w-[560px] flex-col gap-3.5">
              {/* Self-host: the central daemon connects over the CLI. The
                  desktop-download and cloud cards are removed (web-only). */}
              <ForkAlt
