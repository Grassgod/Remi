...
import type { Issue, TimelineEntry } from "@multimira/core/types";
...
vi.mock("@multimira/ui/hooks/use-mobile", () => ({
  useIsMobile: () => mockViewport.isMobile,
}));

// useWorkspaceId() derives from useCurrentWorkspace (relative import inside
// @multimira/core/hooks.tsx). vi.mock("@multimira/core/paths") only intercepts
...
// Mock @multimira/core/workspace/hooks
vi.mock("@multimira/core/workspace/hooks", () => ({
  useActorName: () => ({
...
// Mock @multimira/core/paths — after the URL-driven workspace refactor,
...
vi.mock("@multimira/core/paths", async () => {
  const actual = await vi.importActual<typeof import("@multimira/core/paths")>(
    "@multimira/core/paths",
  );
  return {
    ...actual,
...
// Mock issue config
vi.mock("@multimira/core/issues/config", () => ({
...
vi.mock("@multimira/core/issues/stores", () => ({
...
// Mock realtime
vi.mock("@multimira/core/realtime", () => ({
  useWSEvent: vi.fn(),
...
// Mock react-resizable-panels (used by @multimira/ui/components/ui/resizable)
