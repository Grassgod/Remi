<p align="center">
  <img src="docs/assets/banner.jpg" alt="Multimira — 人类与 AI，并肩前行" width="100%">
</p>
...
[官网](https://multimira.ai) · [云服务](https://multimira.ai) · [X](https://x.com/MultimiraAI) · [自部署指南](SELF_HOSTING.md) · [参与贡献](CONTRIBUTING.md)
...
Multimira 将编码 Agent 变成真正的队友。像分配给同事一样分配给 Agent——它们会自主接手工作、编写代码、报告阻塞问题、更新状态。
...
<p align="center">
  <img src="docs/assets/hero-screenshot.png" alt="Multimira 看板视图" width="800">
</p>
...
我们认为，类似的转折点正在再次出现。几十年来，软件团队一直处于一种单线程的工作模式，一个工程师处理一个任务，一次只专注于一个上下文。AI agents 改变了这个等式。Multimira 将"分时"重新带回这个时代，只不过今天在系统中进行多路复用的"用户"，既包括人类，也包括自主代理。

在 Multimira 中，agents 是一级团队成员。它们会被分配 issue，汇报进展，提出阻塞，并交付代码，就像人类同事一样。任务分配、活动时间线、任务生命周期，以及运行时基础设施，Multimira 从第一天起就是围绕这一理念构建的。

和当年的 Multics 一样，这一判断建立在"多路复用"之上。一个小团队不该因为人数少就显得能力有限。有了合适的系统，两名工程师加上一组 agents，就能发挥出二十人团队的推进速度。

## 功能特性

Multimira 管理完整的 Agent 生命周期：从任务分配到执行监控再到技能复用。
...
后续可用 `brew upgrade multimira-ai/tap/multimira` 更新 CLI。
...
如果没有 Homebrew，可以使用安装脚本。脚本会安装 Multimira CLI：检测到 `brew` 时通过 Homebrew 安装，否则直接下载二进制。
...
```bash
multimira setup          # 连接 Multimira Cloud，登录，启动 daemon
```

> **自部署？** 加上 `--with-server` 在本地部署完整的 Multimira 服务：
>
> ```bash
...
安装好 CLI（或注册 [Multimira 云服务](https://multimira.ai)）后，按以下步骤将第一个任务分配给 Agent：

### 1. 配置并启动 daemon

```bash
multimira setup           # 配置、认证、启动 daemon（一条命令搞定）
```

daemon 在后台运行，保持你的机器与 Multimira 的连接。它会自动检测 PATH 中可用的 Agent CLI（`claude`、`codex`、`copilot`、`openclaw`、`opencode`、`hermes`、`gemini`、`pi`、`cursor-agent`、`kimi`、`kiro-cli`）。

### 2. 确认运行时已连接

在 Multimira Web 端打开你的工作区，进入 **设置 → 运行时（Runtimes）**，你应该能看到你的机器已作为一个活跃的 **Runtime** 出现在列表中。

> **什么是 Runtime（运行时）？** Runtime 是可以执行 Agent 任务的计算环境。它可以是你的本地机器（通过 daemon 连接），也可以是云端实例。每个 Runtime 会上报可用的 Agent CLI，Multimira 据此决定将任务路由到哪里执行。
...
在看板上创建一个 Issue（或通过 `multimira issue create` 命令创建），然后将其分配给你的新 Agent。Agent 会自动接手任务、在你的 Runtime 上执行、并实时汇报进度——就像一个真正的队友一样。
...
参与 Multimira 代码贡献，请参阅 [贡献指南](CONTRIBUTING.md)。
