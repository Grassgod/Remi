...
// loginTokenPrefixes are the token prefixes `multimira login --token` accepts.
// The CLI used to hardcode `mul_` only, which made it impossible to log in
// with a Multimira Cloud Node PAT (`mcn_`) even though the server happily
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate multimira with Multimira",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("MULTIMIRA_TOKEN")); v != "" {
		return v
	}
...
func resolveAppURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'multimira setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'multimira login' to authenticate.")
		return nil
	}
...
<title>Multimira — Authenticated</title>
<style>
