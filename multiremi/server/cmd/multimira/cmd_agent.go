...
// custom_env values — `multimira agent list / get / update` no longer
...
func init() {
	...
	// `multimira agent env set <id>` — that path is owner/admin-only,
...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, MULTIMIRA_SERVER_URL env, or 'multimira config set server_url <url>'")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("MULTIMIRA_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("MULTIMIRA_TASK_ID"); taskID != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "MULTIMIRA_SERVER_URL", "")
	if val != "" {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'multimira setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
// inside a daemon-managed agent task (daemon sets MULTIMIRA_AGENT_ID and
// MULTIMIRA_TASK_ID in the agent env). In that context the workspace must be
// provided explicitly by the daemon — falling back to user-global
// ~/.multimira/config.json would let the agent act on whatever workspace the
...
func inAgentExecutionContext() bool {
	return os.Getenv("MULTIMIRA_AGENT_ID") != "" || os.Getenv("MULTIMIRA_TASK_ID") != ""
}

func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "MULTIMIRA_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inAgentExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: MULTIMIRA_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set MULTIMIRA_WORKSPACE_ID env, or run 'multimira config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `multimira agent env set <id>`)")
	}
