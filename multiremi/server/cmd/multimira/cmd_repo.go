...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("MULTIMIRA_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("MULTIMIRA_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("MULTIMIRA_WORKSPACE_ID")
	agentName := os.Getenv("MULTIMIRA_AGENT_NAME")
	taskID := os.Getenv("MULTIMIRA_TASK_ID")
