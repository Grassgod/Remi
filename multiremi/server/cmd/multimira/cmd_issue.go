...
func init() {
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-Multimira-Next-Before response header; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-Multimira-Next-Before-Id response header; must be paired with --before.")
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	// Quick-create stamp: when the daemon sets MULTIMIRA_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `multimira issue create` call
	...
	if taskID := os.Getenv("MULTIMIRA_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-Multimira-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-Multimira-Next-Before-Id"); nbid != "" {
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set MULTIMIRA_WORKSPACE_ID")
	}
...
// from `multimira workspace member list --output json`) should use this instead of
