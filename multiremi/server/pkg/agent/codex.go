...
const (
	multimiraCodexMcpBeginMarker = "# BEGIN multimira-managed mcp_servers (do not edit; regenerated by daemon)"
	multimiraCodexMcpEndMarker   = "# END multimira-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(multimiraCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(multimiraCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = multimiraCodexMcpBeginMarker + "\n" + multimiraCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(multimiraCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(multimiraCodexMcpEndMarker)
	sb.WriteString("\n")
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "multimira-agent-sdk",
				"title":   "Multimira Agent SDK",
				"version": "0.2.0",
			},
