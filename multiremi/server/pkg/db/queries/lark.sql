...
-- "one Multimira Agent ↔ one Lark Bot"; re-installing on the same agent
...
-- Records that a Lark open_id (per-installation) maps to a Multimira
-- user.
--
...
--   2. ON CONFLICT DO UPDATE is gated on `multimira_user_id` matching
...
INSERT INTO lark_user_binding (
    workspace_id, multimira_user_id, installation_id, lark_open_id, union_id
) VALUES (
...
ON CONFLICT (installation_id, lark_open_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE lark_user_binding.multimira_user_id = EXCLUDED.multimira_user_id
RETURNING *;

-- name: GetLarkUserBindingByOpenID :one
-- The inbound identity check. A row here means: this open_id maps to a
-- Multimira user who IS currently a workspace member (the composite FK
...
-- name: GetLarkUserBindingByUserID :one
-- Reverse lookup for DM notification push: given a Multimira user, find their
-- Lark open_id (+ which installation to send from). Most-recent binding wins
-- when a user is bound across multiple installations. Run `make sqlc` after
-- adding this to regenerate the typed method.
SELECT * FROM lark_user_binding
WHERE multimira_user_id = $1
ORDER BY bound_at DESC
LIMIT 1;
