...
const defaultCLIConfigPath = ".multimira/config.json"
...
// An empty profile returns the default path (~/.multimira/config.json).
// A named profile returns ~/.multimira/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".multimira", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.multimira/. A named profile returns ~/.multimira/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".multimira"), nil
	}
	return filepath.Join(home, ".multimira", "profiles", profile), nil
}
