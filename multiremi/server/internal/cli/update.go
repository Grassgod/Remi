...
func releaseAssetCandidates(targetVersion, goos, goarch string) []string {
	...
	// `multimira_{os}_{arch}` name for releases that still ship it.
	return []string{
		fmt.Sprintf("multimira-cli-%s-%s-%s.%s", version, goos, goarch, ext),
		fmt.Sprintf("multimira_%s_%s.%s", goos, goarch, ext),
	}
}
...
func fetchReleaseByTag(tag string) (*GitHubRelease, error) {
	...
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/multimira-ai/multimira/releases/tags/"+tag, nil)
	if err != nil {
		return nil, err
	}
...
// FetchLatestRelease fetches the latest release tag from the multimira GitHub repo.
func FetchLatestRelease() (*GitHubRelease, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/multimira-ai/multimira/releases/latest", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Accept", "application/vnd.github+json")
...
// IsBrewInstall checks whether the running multimira binary was installed via Homebrew.
...
// UpdateViaBrew runs `brew upgrade multimira-ai/tap/multimira`.
// Returns the combined output and any error.
func UpdateViaBrew() (string, error) {
	cmd := exec.Command("brew", "upgrade", "multimira-ai/tap/multimira")
...
func UpdateViaDownloadWithTimeout(targetVersion string, downloadTimeout time.Duration) (string, error) {
	...
	binaryName := "multimira"
	if runtime.GOOS == "windows" {
		binaryName = "multimira.exe"
	}
	var binaryData []byte
	if runtime.GOOS == "windows" {
	...
	tmpFile, err := os.CreateTemp(dir, "multimira-update-*")
	if err != nil {
