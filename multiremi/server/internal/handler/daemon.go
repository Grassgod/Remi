...
type DaemonRegisterRequest struct {
	...
	CLIVersion      string   `json:"cli_version"` // multimira CLI version
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	if agent, err := h.Queries.GetAgent(r.Context(), task.AgentID); err == nil {
		...
		// names carry a "multimira-" prefix so their on-disk slugs never collide
			...
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					...
					for _, row := range rows {
						...
						// so `multimira repo checkout` and the meta-skill render
	...
	if task.ChatSessionID.Valid {
		if cs, err := h.Queries.GetChatSession(r.Context(), task.ChatSessionID); err == nil {
			...
			// the agent can `multimira attachment download <id>` — the markdown
	...
	if task.Context != nil && !task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid {
		...
		if json.Unmarshal(task.Context, &qc) == nil && qc.Type == service.QuickCreateContextType {
			...
			// the project, and `multimira repo checkout` sees the project's
	...
	// as the only authority for MULTIMIRA_WORKSPACE_ID in the agent env. An
	...
	// owner). The daemon will inject this as MULTIMIRA_TOKEN into the agent
