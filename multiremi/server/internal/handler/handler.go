...
type Config struct {
	...
	// public internet, with no trailing slash (e.g. "https://app.multimira.ai").
	...
	// MULTIMIRA_TRUSTED_PROXIES env var (comma-separated CIDRs, e.g.
...
type Handler struct {
	...
	// (MULTIMIRA_LARK_SECRET_KEY) is unset; the corresponding HTTP
	...
	// MULTIMIRA_LARK_SECRET_KEY is set; tests that need a no-op
	...
	// when the master at-rest key (MULTIMIRA_LARK_SECRET_KEY) is unset.
