...
// translates the prompt into a `multimira issue create` invocation in the
...
// the agent's `multimira issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the multimira CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See multimira-ai/multimira#1660.
