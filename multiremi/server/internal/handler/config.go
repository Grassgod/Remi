...
type AppConfig struct {
	...
	// self-hosted instances can show `multimira setup self-host` commands
	// with the operator's own domains instead of Multimira Cloud defaults.
...
func daemonSetupURLsFromEnv() (string, string) {
	serverURL := normalizePublicURL(os.Getenv("MULTIMIRA_PUBLIC_URL"))
	appURL := normalizePublicURL(os.Getenv("MULTIMIRA_APP_URL"))
	if appURL == "" {
...
// Multimira Cloud, identified by its frontend host alone (multimira.ai /
// app.multimira.ai). The daemon setup for the managed cloud is always
// `multimira setup` (which hardcodes api.multimira.ai), so the per-deployment URLs
// must be omitted from /api/config even when MULTIMIRA_PUBLIC_URL is unset or
// misconfigured. Previously this also required serverURL==api.multimira.ai, so a
// cloud deployment that forgot MULTIMIRA_PUBLIC_URL fell through and emitted a
// `setup self-host --server-url https://multimira.ai` command — pointing the
// daemon's backend at the frontend (no /health, no WebSocket proxy).
func isOfficialCloudDaemonConfig(appURL string) bool {
	return urlHostEquals(appURL, "multimira.ai") || urlHostEquals(appURL, "app.multimira.ai")
}
