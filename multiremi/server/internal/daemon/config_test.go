...
func TestPatternsFromEnv_DefaultsWhenUnset(t *testing.T) {
	t.Setenv("MULTIMIRA_GC_ARTIFACT_PATTERNS", "")
	defaults := []string{"node_modules", ".next", ".turbo"}
	got := patternsFromEnv("MULTIMIRA_GC_ARTIFACT_PATTERNS", defaults)
...
func TestPatternsFromEnv_DropsSeparatorBearingEntries(t *testing.T) {
	t.Setenv("MULTIMIRA_GC_ARTIFACT_PATTERNS", "node_modules, .next ,foo/bar, ../etc, ,target")
	got := patternsFromEnv("MULTIMIRA_GC_ARTIFACT_PATTERNS", nil)
...
func TestIsOfficialCloudServer(t *testing.T) {
	for _, tc := range []struct {
		...
		{"canonical cloud https", "https://api.multimira.ai", true},
		{"canonical cloud with trailing slash stripped", "https://api.multimira.ai/", true},
		{"canonical cloud case-insensitive", "https://API.Multimira.AI", true},
		{"cloud over plain http (unusual but match host)", "http://api.multimira.ai", true},
		...
		{"third-party host is self-host", "https://multimira.example.com", false},
		// Staging / preview / future subdomains deliberately follow the
		// safer self-host default until explicitly opted in.
		{"multimira.ai apex is not the api host", "https://multimira.ai", false},
		{"staging subdomain is self-host", "https://staging.multimira.ai", false},
		{"preview subdomain is self-host", "https://api-preview.multimira.ai", false},
...
func stageFakeAgent(t *testing.T) string {
	...
	t.Setenv("MULTIMIRA_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	...
	t.Setenv("MULTIMIRA_DAEMON_AUTO_UPDATE", "")
	return binDir
}
...
// daemon pointed at Multimira's hosted cloud keeps the historical opt-in
...
func TestLoadConfig_AutoUpdateDefault_CloudOn(t *testing.T) {
	stageFakeAgent(t)
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "wss://api.multimira.ai/ws",
		WorkspacesRoot: t.TempDir(),
	})
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	...
	if !cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = false for Multimira Cloud server, want true")
	}
}
...
func TestLoadConfig_AutoUpdateEnv_ForcesOnForSelfHost(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("MULTIMIRA_DAEMON_AUTO_UPDATE", "true")
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
	...
	if !cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = false after explicit MULTIMIRA_DAEMON_AUTO_UPDATE=true, want true")
	}
}
...
func TestLoadConfig_AutoUpdateEnv_ForcesOffForCloud(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("MULTIMIRA_DAEMON_AUTO_UPDATE", "false")
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "https://api.multimira.ai",
		WorkspacesRoot: t.TempDir(),
	})
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	...
	if cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = true after explicit MULTIMIRA_DAEMON_AUTO_UPDATE=false, want false")
	}
}
...
// resolvable via the daemon's PATH (or pinned to an explicit MULTIMIRA_*_PATH),
...
func TestLoadConfig_UsesCodexDesktopAppBundleFallback(t *testing.T) {
	...
	t.Setenv("MULTIMIRA_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	t.Setenv("MULTIMIRA_CODEX_MODEL", "gpt-5")
...
func TestLoadConfig_CodexDesktopFallbackDoesNotOverrideExplicitPath(t *testing.T) {
	...
	t.Setenv("SHELL", filepath.Join(t.TempDir(), "fish"))
	t.Setenv("MULTIMIRA_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	t.Setenv("MULTIMIRA_CODEX_PATH", filepath.Join(t.TempDir(), "missing-codex"))
	pinNonCodexAgentsToMissingPaths(t)
	fakeClaude := filepath.Join(t.TempDir(), "claude")
	if err := os.WriteFile(fakeClaude, []byte("#!/bin/sh\nexit 0\n"), 0o755); err != nil {
		t.Fatalf("write fake claude: %v", err)
	}
	t.Setenv("MULTIMIRA_CLAUDE_PATH", fakeClaude)
	...
	if got, ok := cfg.Agents["codex"]; ok {
		t.Fatalf("explicit missing MULTIMIRA_CODEX_PATH should not fall back to Desktop bundle, got %#v", got)
	}
}
...
func pinNonCodexAgentsToMissingPaths(t *testing.T) {
	...
	for _, name := range []string{
		"MULTIMIRA_CLAUDE_PATH",
		"MULTIMIRA_OPENCODE_PATH",
		"MULTIMIRA_OPENCLAW_PATH",
		"MULTIMIRA_HERMES_PATH",
		"MULTIMIRA_GEMINI_PATH",
		"MULTIMIRA_PI_PATH",
		"MULTIMIRA_CURSOR_PATH",
		"MULTIMIRA_COPILOT_PATH",
		"MULTIMIRA_KIMI_PATH",
		"MULTIMIRA_KIRO_PATH",
	} {
