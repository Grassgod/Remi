...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "multimira login") {
		t.Fatalf("401 warning must tell the user to run 'multimira login', got: %s", out)
	}
}
...
// and the operator would never see a "run multimira login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "multimira login") {
		t.Fatalf("expected the actionable 'run multimira login' hint in the WARN, got: %s", out)
	}
}
