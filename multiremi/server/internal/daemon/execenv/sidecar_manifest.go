...
const sidecarManifestFile = ".multimira_sidecar_manifest.json"
...
//     so the agent still discovers the Multimira skill, just under a
//     different directory name.
//   - For Multimira-only namespaces (.agent_context/issue_context.md,
//     .multimira/project/resources.json) the caller swallows the error
...
// down a Multimira skill without colliding with a user-installed skill
...
// collision we append `-multimira`, then `-multimira-2`, `-multimira-3`,
...
func allocateCollisionFreeSkillDir(skillsParent, baseSlug string) (slug, dir string, err error) {
	...
	for i := 0; i < maxAttempts; i++ {
		...
		case i == 0:
			candidate = baseSlug
		case i == 1:
			candidate = baseSlug + "-multimira"
		default:
			candidate = fmt.Sprintf("%s-multimira-%d", baseSlug, i)
		}
...
// (.agent_context/, .multimira/, .claude/skills/, .github/skills/,
...
// issue-review-multimira. A managed skill directory is platform-owned — the
