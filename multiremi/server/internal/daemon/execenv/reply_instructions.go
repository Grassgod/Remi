	...
	if threadID != "" {
		return fmt.Sprintf(
			"%d new comment(s) on this issue since your last run — don't read them all blindly. "+
				...
				"`multimira issue comment list %s --thread %s --since %s --output json` "+
				...
				"`multimira issue comment list %s --since %s --output json`.\n\n",
...
func BuildResumedCommentsHint(issueID, triggerCommentID, triggerThreadID string) string {
	...
	return fmt.Sprintf(
		"You're resuming the prior session, and the triggering comment is already included above. "+
			...
			"`multimira issue comment list %s --thread %s --tail 30 --output json`.\n\n",
...
func BuildColdCommentsHint(issueID, triggerCommentID, triggerThreadID string) string {
	...
	return fmt.Sprintf(
		"Read the triggering conversation first: "+
			...
			"Need cross-thread background? `multimira issue comment list %s --recent 20 --output json`.\n\n",
...
func BuildCommentReplyInstructions(provider, issueID, triggerCommentID string) string {
	...
	if runtimeGOOS == "windows" {
		return fmt.Sprintf(
			"If you decide to reply, post it as a comment — always use the trigger comment ID below, "+
				...
				"Do NOT pipe via `--content-stdin` — Windows PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to native commands and silently drops non-ASCII (Chinese, Japanese, Cyrillic, accents, emoji) as `?` before the bytes reach `multimira.exe`. "+
				...
				"    # 2. Then run:\n"+
				"    multimira issue comment add %s --parent %s --content-file ./reply.md\n\n"+
	...
	return fmt.Sprintf(
		"If you decide to reply, post it as a comment — always use the trigger comment ID below, "+
			...
			"Use this form, preserving the same issue ID and --parent value:\n\n"+
			"    cat <<'COMMENT' | multimira issue comment add %s --parent %s --content-stdin\n"+
			"    First paragraph.\n"+
			"\n"+
			"    Second paragraph.\n"+
			"    COMMENT\n\n"+
