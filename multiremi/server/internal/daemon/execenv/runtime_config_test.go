	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				"`--status backlog` = **wait**",
				"`multimira issue status <child-id> todo`",
				"all `--status todo`",
...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"multimira issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`multimira issue status <this-issue-id> in_review`",
...
// dedicated protocol step that wrote `multimira issue status <this-issue-id> in_review`;
...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	if strings.Contains(out, "`multimira issue status <this-issue-id> in_review`") {
...
func TestCommentTriggeredBriefCarriesNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "multimira issue comment list "+issueID+" --since "+since+" --output json") {
		t.Errorf("comment brief must keep the issue-wide --since catch-up fallback, got:\n%s", out)
	}
	// The removed resolve step must not reappear.
	if strings.Contains(out, "multimira comment resolve") {
...
func TestCommentTriggeredBriefColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "multimira issue comment list "+issueID+" --thread thread-root-1 --tail 30 --output json") {
...
func TestAssignmentTriggeredProtocolHonorsAgentIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Run `multimira issue status " + issueID + " in_progress` unless your Agent Identity forbids issue status changes; if it does, skip this step.",
		...
		"When done, run `multimira issue status " + issueID + " in_review` unless your Agent Identity forbids issue status changes; if it does, skip this step.",
		"If blocked, run `multimira issue status " + issueID + " blocked` unless your Agent Identity forbids issue status changes.",
	} {
		if !strings.Contains(out, want) {
	...
	for _, banned := range []string{
		"4. Run `multimira issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `multimira issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestWriteRuntimeConfigFileCreatesMissingFile(t *testing.T) {
	...
	const brief = "# Multimira Agent Runtime\n\nbrief body line"
...
func TestWriteRuntimeConfigFileReplacesExistingBlock(t *testing.T) {
	...
	const newBrief = "## New Multimira brief\n\nfresh body"
...
func TestWriteRuntimeConfigFileIgnoresStrayEndMarkerBeforeBegin(t *testing.T) {
	...
	// real block authored by an earlier Multimira run below.
	const userDoc = "# Repo CLAUDE.md\n\nExample of what Multimira writes:\n" +
		runtimeMarkerEnd + "\n\n# Real config below\n"
	original := userDoc +
		runtimeMarkerBegin + "\nFIRST BRIEF\n" + runtimeMarkerEnd + "\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
...
// `codex` run started by the user after a Multimira task must see the same
// file the user wrote.
