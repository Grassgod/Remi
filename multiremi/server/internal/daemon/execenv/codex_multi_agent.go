...
// subagents. The Multimira daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Multimira runtime
...
//
// Users who explicitly want Codex native subagents inside a Multimira task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `MULTIMIRA_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// MultimiraCodexMultiAgentEnv is the env var users can set to keep Codex
...
const MultimiraCodexMultiAgentEnv = "MULTIMIRA_CODEX_MULTI_AGENT"

// multimiraMultiAgentBeginMarker / multimiraMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	multimiraMultiAgentBeginMarker = "# BEGIN multimira-managed multi-agent (do not edit; regenerated by daemon)"
	multimiraMultiAgentEndMarker   = "# END multimira-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(multimiraMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(multimiraMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(MultimiraCodexMultiAgentEnv))
...
// renderMultimiraMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderMultimiraMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(multimiraMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(multimiraMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderMultimiraMultiAgentBlock(true)
...
//
// When MULTIMIRA_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per MULTIMIRA_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderMultimiraMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
