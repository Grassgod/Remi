...
func TestEnsureCodexMultiAgentConfigEmptyFile(t *testing.T) {
	...
	if !strings.Contains(got, multimiraMultiAgentBeginMarker) {
		t.Errorf("expected begin marker, got:\n%s", got)
	}
	if !strings.Contains(got, multimiraMultiAgentEndMarker) {
...
`
	...
	t.Setenv(MultimiraCodexMultiAgentEnv, "1")
...
func TestCodexMultiAgentEnabledTruthy(t *testing.T) {
	for _, v := range []string{"1", "true", "TRUE", "yes", "On"} {
		t.Run(v, func(t *testing.T) {
			t.Setenv(MultimiraCodexMultiAgentEnv, v)
...
`
	...
	data, _ := os.ReadFile(configPath)
	got := string(data)
	if !strings.Contains(got, multimiraManagedBeginMarker) {
