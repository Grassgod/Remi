...
// — users cannot audit or edit the contents from any Multimira UI.
//
// This conflicts with the Multimira daemon's context model. Multimira
...
//     unrelated host-project context into Multimira tasks. The reproduction
//     in github.com/multimira-ai/multimira#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Multimira issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Multimira task
// (and accept the leak risk) can keep the feature enabled by setting
// `MULTIMIRA_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Multimira-owned, user-visible,
...
// MultimiraCodexMemoryEnv is the env var users can set to keep Codex
...
const MultimiraCodexMemoryEnv = "MULTIMIRA_CODEX_MEMORY"

const (
	multimiraMemoryFeatureBeginMarker = "# BEGIN multimira-managed memory-feature (do not edit; regenerated by daemon)"
	multimiraMemoryFeatureEndMarker   = "# END multimira-managed memory-feature"
	multimiraMemoryConfigBeginMarker  = "# BEGIN multimira-managed memory-config (do not edit; regenerated by daemon)"
	multimiraMemoryConfigEndMarker    = "# END multimira-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(multimiraMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(multimiraMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(multimiraMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(multimiraMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(MultimiraCodexMemoryEnv))
...
// renderMultimiraMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderMultimiraMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(multimiraMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(multimiraMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderMultimiraMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderMultimiraMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(multimiraMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(multimiraMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderMultimiraMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderMultimiraMemoryConfigBlock(true))
}
...
//
// When MULTIMIRA_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per MULTIMIRA_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderMultimiraMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderMultimiraMemoryConfigBlock(false))
	}
