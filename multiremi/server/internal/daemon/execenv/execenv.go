...
// checked out on demand by the agent via `multimira repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string // base path for all envs (e.g., ~/multimira_workspaces)
...
// on demand via `multimira repo checkout <url>`.
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	// a collision-free sibling (issue-review, issue-review-multimira,
	// issue-review-multimira-2, …), accumulating a fresh duplicate on every
	...
	//      refresh back to issue-review-multimira.
