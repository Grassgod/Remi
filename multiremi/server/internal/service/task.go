...
import (
	...
	"github.com/multimira-ai/multimira/server/internal/mention"
	...
	"github.com/multimira-ai/multimira/server/pkg/redact"
...
type TaskService struct {
	...
	// DMNotifier, when set, pushes a best-effort Feishu DM to a member after an
	// inbox notification is created (e.g. "your agent finished MUL-123"). Kept
	// as a plain func so the service package stays decoupled from the lark
	// package (which imports service). Wired in router.go to
	// lark.UserNotifier.NotifyUser. Nil = no DM push. Must never block or fail
	// the caller — implementations log and swallow errors.
	DMNotifier func(ctx context.Context, userID pgtype.UUID, title, body string)
...
// `multimira issue create` call. Pre-validates that the agent is reachable
...
// manual `multimira agent update <id> --status idle` to unwedge. Matches the
...
// daemon-injected MULTIMIRA_QUICK_CREATE_TASK_ID env var, so this lookup is
...
func (s *TaskService) notifyQuickCreateCompleted(ctx context.Context, task db.AgentTaskQueue, qc QuickCreateContext) {
	...
	if s.DMNotifier != nil {
		// Fire-and-forget Feishu DM. Detached context so it survives the
		// request lifecycle; the notifier itself swallows all errors.
		go s.DMNotifier(context.Background(), requesterID, issue.Title, "Your agent finished this task.")
	}
}
...
func (s *TaskService) notifyQuickCreateFailed(ctx context.Context, task db.AgentTaskQueue, qc QuickCreateContext, errMsg string) {
	...
	if s.DMNotifier != nil {
		go s.DMNotifier(context.Background(), requesterID, "Quick create failed", redact.Text(errMsg))
	}
}
