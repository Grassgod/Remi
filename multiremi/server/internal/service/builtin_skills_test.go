...
func TestBuiltinSkillsConformToTemplate(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			// The multimira- prefix keeps the on-disk slug from colliding with a
			// user-authored workspace skill.
			if !strings.HasPrefix(skill.Name, "multimira-") {
				t.Errorf("skill name %q must carry the multimira- prefix", skill.Name)
			}
...
// must declare user-invocable:false and fence itself to the multimira CLI. New
// contract skills should copy this shape.
func TestMentioningSkillFollowsContractFrontmatter(t *testing.T) {
	skill, ok := findSkill(t, "multimira-mentioning")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); got != "Bash(multimira *)" {
		t.Errorf("allowed-tools = %q, want Bash(multimira *) (fence the skill to the CLI it teaches)", got)
	}
}
...
func TestWorkingOnIssuesSkillCoversIssueLoopContracts(t *testing.T) {
	skill, ok := findSkill(t, "multimira-working-on-issues")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); !strings.Contains(got, "Bash(multimira *)") {
		t.Errorf("allowed-tools = %q, want access to the Multimira CLI", got)
	}
	...
	mustContain := []string{
		"multimira issue pull-requests <issue-id> --output json",
		"Default for code-changing issue work",
		"open or update a PR before posting the final Multimira issue comment",
	...
	mustNotContain := []string{
		...
		"multimira issue get <issue-id> --output json",
		"multimira issue metadata list <issue-id> --output json",
		"multimira issue comment list <issue-id> --thread <trigger-comment-id>",
		"multimira issue comment add <issue-id> --parent <trigger-comment-id>",
	}
...
func TestSkillImportingSkillCoversWorkspaceImportContracts(t *testing.T) {
	skill, ok := findSkill(t, "multimira-skill-importing")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		"multimira skill import --url <url> --output json",
		"/api/skills/import",
		"clawhub.ai",
		"skills.sh",
		"github.com",
		...
		"name",
		"legacy",
		"multimira skill list --output json",
		"npx skills add",
		"multimira agent skills add <agent-id> --skill-ids <skill-id> --output json",
		"multimira agent skills list <agent-id> --output json",
		"replace-all",
	...
	mustNotContain := []string{
		"multimira agent skills set <agent-id> --skill-ids <skill-id>",
...
func TestCreatingAgentsSkillCoversAgentCreationContracts(t *testing.T) {
	skill, ok := findSkill(t, "multimira-creating-agents")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"multimira agent create --name <name> --runtime-id <runtime-id>",
		...
		"multimira agent skills add <agent-id> --skill-ids <skill-id> --output json",
		"multimira agent skills list <agent-id> --output json",
		"multimira agent get <agent-id> --output json",
		"255",
...
func TestSquadsSkillCoversLeaderRoutingContract(t *testing.T) {
	skill, ok := findSkill(t, "multimira-squads")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"multimira squad member set-role",
...
func TestAutopilotsSkillCoversDispatchAndSideEffects(t *testing.T) {
	skill, ok := findSkill(t, "multimira-autopilots")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"create_issue",
		"run_only",
		"multimira autopilot trigger-add <autopilot-id> --kind schedule",
		"multimira autopilot trigger <autopilot-id> --output json",
		"Do not run `trigger`",
		"webhook tokens",
		"{{date}}",
...
func TestRuntimesAndReposSkillCoversClaimAndCheckoutChain(t *testing.T) {
	skill, ok := findSkill(t, "multimira-runtimes-and-repos")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"multimira runtime list --output json",
		"multimira repo checkout <url>",
		"MULTIMIRA_DAEMON_PORT",
		"github_repo",
		"local_directory",
...
func TestProjectsAndResourcesSkillCoversDurableContext(t *testing.T) {
	skill, ok := findSkill(t, "multimira-projects-and-resources")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"multimira project resource list <project-id> --output json",
		"multimira project resource add <project-id> --type github_repo --url <github-url> --output json",
		"multimira project resource add <project-id> --type local_directory",
