---
name: multimira-skill-importing
description: Use when a user provides a skill URL, slug, or clear intent to import/install a specific skill into the current Multimira workspace. Teaches the workspace import API/CLI path (POST /api/skills/import), the supported URL source families, the SkillWithFilesResponse shape returned on success, duplicate 409 handling with the existing_skill body, additive agent binding vs replace-all, and the reserved SKILL.md supporting-file rule. Do not use it to decide which skill the user needs, and never treat an external local installer like npx skills add as the final Multimira install.
user-invocable: false
allowed-tools: Bash(multimira *)
---

# Importing skills into Multimira

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Multimira workspace.
...
A skill is installed for Multimira only when it exists in the current workspace's
...
environment, not the Multimira workspace DB, so Multimira cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `multimira skill import --url <url> --output json`:

```bash
multimira skill import --url clawhub.ai/owner/skill --output json
multimira skill import --url skills.sh/owner/repo/skill --output json
multimira skill import --url github.com/owner/repo --output json
multimira skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
multimira skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
After the final `multimira agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`multimira agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`multimira agent skills set` is replace-all: the server clears every current
...
`multimira skill import --url <url> --output json` prints that structured conflict
...
```bash
multimira skill get <skill-id> --output json
```
...
Incorrect (bypasses Multimira):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Multimira cannot manage it as a workspace skill.
...
```bash
multimira skill import --url https://skills.sh/owner/repo/skill --output json
```
