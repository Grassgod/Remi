...
This skill states WHAT a mention link does in the Multimira backend, traced to
...
- a person → `multimira workspace member list --output json` → use `user_id`
- an agent → `multimira agent list --output json` → use `id`
- a squad  → `multimira squad list --output json` → use `id`
...
Correct:
  1. `multimira workspace member list --output json`  → Alice's `user_id` = 7f3a…
