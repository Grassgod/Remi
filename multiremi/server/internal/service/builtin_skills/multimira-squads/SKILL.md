---
name: multimira-squads
description: Use when creating, inspecting, updating, assigning, mentioning, or debugging Multimira squads. Explains what squads are, squad/member fields, CLI commands, leader routing, issue assignment, comments, mentions, autopilot behavior, leader briefing, side effects, and product-gap handling.
...
multimira squad get <squad-id> --output json
multimira squad member list <squad-id> --output json
multimira issue comment list <issue-id> --recent 20 --output json
```

If the command shape is unclear, check help instead of guessing:

```bash
multimira squad --help
multimira squad member --help
multimira issue update --help
...
A Multimira squad is a workspace routing and coordination object.
...
```bash
multimira squad list --output json
multimira squad get <squad-id> --output json
multimira squad create --name <name> --leader <agent-name-or-id> --output json
multimira squad update <squad-id> --instructions "<leader coordination policy>" --output json
multimira squad delete <squad-id>
```
...
```bash
multimira squad member list <squad-id> --output json
multimira squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
multimira squad member remove <squad-id> --member-id <id> --type agent|member
multimira squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
multimira squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
multimira issue get <issue-id> --output json
multimira issue update <issue-id> --help
multimira issue comment list <issue-id> --output json
multimira issue comment add <issue-id> --help
```
...
For squad leader tasks, Multimira appends a squad leader briefing to the leader
...
- recording squad activity with `multimira squad activity`;
