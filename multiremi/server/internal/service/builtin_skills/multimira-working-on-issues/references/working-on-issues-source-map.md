...
## `multimira issue pull-requests` — read PR links from Multimira

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/multimira/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/multimira/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/multimira/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `multimira issue pull-requests` later reads back.
...
| Behavior | File:line |
|---|---|
| `multimira issue metadata set <issue-id> --key --value [--type]` | `server/cmd/multimira/cmd_issue_metadata.go:80,109-111` |
| `multimira issue metadata delete <issue-id> --key` | `server/cmd/multimira/cmd_issue_metadata.go:93,113` |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/multimira/cmd_issue.go
