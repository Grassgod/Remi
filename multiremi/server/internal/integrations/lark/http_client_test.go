...
// TestHTTPClient_SendMarkdownDM_HappyPath verifies the DM variant targets a
// user's p2p chat (receive_id_type=open_id) rather than a group chat, reusing
// the same schema-2.0 markdown card envelope as SendMarkdownCard.
func TestHTTPClient_SendMarkdownDM_HappyPath(t *testing.T) {
	fake := newLarkFake(t)
	...
	fake.stubSend(
		...
		func(r *http.Request, body map[string]string) {
			...
			if body["receive_id"] != "ou_user_99" {
				t.Errorf("receive_id: got %q want ou_user_99", body["receive_id"])
			}
			if body["msg_type"] != "interactive" {
				t.Errorf("msg_type: got %q want interactive", body["msg_type"])
			}
	...
	c := newTestClient(fake, time.Now)
	msgID, err := c.SendMarkdownDM(context.Background(), testCreds(), "ou_user_99", "**Done**\nYour agent finished.", "Done")
	if err != nil {
		t.Fatalf("send markdown dm: %v", err)
	}
	if msgID != "om_dm_1" {
		t.Errorf("message id: got %q want om_dm_1", msgID)
	}
	if got := fake.lastAuth(); got != "Bearer tok_dm" {
		t.Errorf("Authorization header: got %q want Bearer tok_dm", got)
	}
}

// TestHTTPClient_SendMarkdownDM_MissingOpenID fails fast without a recipient.
func TestHTTPClient_SendMarkdownDM_MissingOpenID(t *testing.T) {
	fake := newLarkFake(t)
	c := newTestClient(fake, time.Now)
	if _, err := c.SendMarkdownDM(context.Background(), testCreds(), "", "body", ""); err == nil {
...
func TestHTTPClient_SendBindingPromptCard_HappyPath(t *testing.T) {
	...
	if err := c.SendBindingPromptCard(context.Background(), BindingPromptParams{
		...
		BindURL:        "https://multimira.test/lark/bind?token=abc",
	}); err != nil {
	...
	if !strings.Contains(capturedBody["content"], "multimira.test/lark/bind") {
...
func TestBindingPromptTemplate_Shape(t *testing.T) {
	raw, err := bindingPromptTemplate("https://multimira.test/bind?token=abc")
	if err != nil {
	...
	if btn["url"] != "https://multimira.test/bind?token=abc" {
