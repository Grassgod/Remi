...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	MultimiraUserID  pgtype.UUID // the installer's Multimira account
...
// identity is the supplied multimiraUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Multimira user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, multimiraUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
	...
	_, err = qtx.CreateLarkUserBinding(ctx, db.CreateLarkUserBindingParams{
		...
		MultimiraUserID:  multimiraUserID,
		InstallationID: row.InstallationID,
		LarkOpenID:     row.LarkOpenID,
	})
	if err != nil {
		// pgx.ErrNoRows here means the conflict row exists but its
		// multimira_user_id differs from ours, so the WHERE clause on
		...
		// lark_user_binding_member_fk (workspace_id, multimira_user_id)
...
// (installation_id, lark_open_id) when multimira_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *db.Queries, p InstallerBindParams) error {
	...
	_, err := q.CreateLarkUserBinding(ctx, db.CreateLarkUserBindingParams{
		...
		MultimiraUserID:  p.MultimiraUserID,
...
// open_id) pair and points at a different Multimira user. Account
