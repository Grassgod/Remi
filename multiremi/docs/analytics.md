...
This document is the source of truth for the analytics events Multimira ships
...
See [MUL-1122](https://github.com/multimira-ai/multimira) for the design context.
...
Self-hosters should **never inherit a Multimira-issued `POSTHOG_API_KEY`** —
...
- **Person properties (`$set`)** — use for mutable cohort signals
  ...
  `setPersonProperties()` in `@multimira/core/analytics`. Use
...
Grafana via `multimira_*` business counters (see `server/internal/metrics`).
...
| `signup_source` | string | Opaque attribution bundle from the frontend cookie `multimira_signup_source` (UTM + referrer). Empty when the cookie is absent. |
...
| `cli_version` | string | Version of the `multimira` CLI that registered it. |
...
| Metric | Type | Labels |
|---|---|---|
| `multimira_agent_task_enqueued_total` | counter | `source`, `runtime_mode` |
| `multimira_agent_task_dispatched_total` | counter | `source`, `runtime_mode` |
| `multimira_agent_task_started_total` | counter | `source`, `runtime_mode`, `provider` |
| `multimira_agent_task_terminal_total` | counter | `source`, `runtime_mode`, `terminal_status` |
| `multimira_agent_task_failed_total` | counter | `source`, `runtime_mode`, `failure_reason` |
| `multimira_agent_task_queue_wait_seconds` | histogram | `source`, `runtime_mode` |
| `multimira_agent_task_run_seconds` | histogram | `source`, `runtime_mode`, `terminal_status` |
| `multimira_agent_task_total_seconds` | histogram | `source`, `runtime_mode`, `terminal_status` |
...
- `terminal_status` is the task's final `agent_task_queue.status` —
  ...
  `multimira_agent_task_terminal_total{terminal_status=…}`. Failures
  additionally increment `multimira_agent_task_failed_total` carrying the coarse
...
- Attribution is NOT a separate event; UTM + referrer origin are written
  to the `multimira_signup_source` cookie on the first anonymous pageview
...
`BusinessMetrics.RecordTaskTerminal` counter (exported as a `multimira_*` task
