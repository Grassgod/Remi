...
- 全部通过 → 创建 MR → Mission 进入 `in_review`
...
1. 创建 MR（从执行分支合入到项目配置的目标分支）。根据仓库地址判断平台：

**GitHub** (github.com)：
```bash
MR_URL=$(gh pr create --base {releaseBranch} --head {executeBranch} --title "Mission: {missionTitle}" --body "Contract 验证通过，请审核合入。" 2>&1 | grep -o 'https://[^ ]*')
```
...
3. 调用 `mission-advance` 推进状态并记录 MR：
```bash
bun .claude/skills/contract-eval/mission-advance {missionId} pass {MR_URL}
```

#### 未通过时：
```bash
bun .claude/skills/contract-eval/mission-advance {missionId} fail "失败原因"
```

**这是唯一的结果判定方式。** 脚本直接更新 Mission 状态。不调用此脚本 = pipeline 不会推进。
