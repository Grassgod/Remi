...
- 全部通过 → Mission 自动进入 `in_review`（由 auto-advance 检测 eval-report.md）
- 任一失败 → Mission 自动回到 `execute` 重试

**注意：** MR 创建不在 eval 阶段进行。用户在 Dashboard 上 review 后通过 "Create MR" 按钮创建。
