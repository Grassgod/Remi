...
验证完成后，**必须**通过 Bash 调用 `mission-advance` 脚本宣告结果：

通过时：
```bash
bun .claude/skills/contract-eval/mission-advance {missionId} pass
```
...
**这是唯一的结果判定方式。** 脚本直接更新 Mission 状态并触发下一步。不调用此脚本 = pipeline 不会推进。
