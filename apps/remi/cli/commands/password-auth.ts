...
import { CliApiClient, CliError, type CliOptionSpec, type CommandInvocation, type CommandSpec } from "../core/index.js";
import {
  INPUT_OPTIONS,
  commandOptions,
  integerOption,
...
export function passwordLoginCommandSpec(): CommandSpec {
  return {
    ...
    run: async (invocation) => {
      // Public login must not send a previous server's credentials to the
      // selected server, including when --server switches CLI profiles.
