...
This guide documents the local development workflow for contributors working on the Multimira codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=multimira`
...
```bash
POSTGRES_DB=multimira_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://multimira:multimira@localhost:5432/multimira_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../multimira-feature -b feat/my-change main
cd ../multimira-feature
make dev
```
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `multimira_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`multimira login`).
...
whatever server is configured in `~/.multimira/config.json`. That's fine for
...
Example: worktree at `../multimira-feat-auth` produces profile
`dev-multimira_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `MULTIMIRA_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.multimira/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `multimira` commands automatically use the same
...
rm -rf "$HOME/.multimira/profiles/$PROFILE"
```
...
1. Compiles the `multimira` CLI from `server/cmd/multimira` into
   `apps/desktop/resources/bin/multimira`
...
backend logs. If you set `MULTIMIRA_DEV_VERIFICATION_CODE=888888` before starting
...
Nothing in this flow touches the system-installed `multimira` or the default
`~/.multimira/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.multimira/config.json` | `~/.multimira/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.multimira/daemon.pid` | `~/.multimira/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/multimira_workspaces/` | `~/multimira_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `multimira_<slug>_<hash>` |
| Desktop profile | `desktop-api.multimira.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U multimira -d postgres -At -c "select datname from pg_database order by datname;"
```
