...
import { ApiError } from "@multimira/core/api";
...
vi.mock("@multimira/ui/components/ui/sidebar", () => ({
...
vi.mock("@multimira/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@multimira/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));

vi.mock("@multimira/core/auth", () => ({
...
vi.mock("@multimira/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@multimira/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@multimira/core/inbox/queries", () => ({ deduplicateInboxItems: (items: unknown[]) => items, inboxKeys: { list: () => ["inbox"] } }));
vi.mock("@multimira/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@multimira/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@multimira/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@multimira/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@multimira/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@multimira/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@multimira/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@multimira/core/runtimes/hooks", () => ({ useMyRuntimesNeedUpdate: () => false }));
