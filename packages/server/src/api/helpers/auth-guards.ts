...
export function isTaskTokenForbiddenRequest(request: Request): boolean {
  ...
  if (path.startsWith("/api/workspaces/")) {
    ...
    if (/^\/api\/workspaces\/[^/]+\/feishu\/messages\/[^/]+\/(notify|draft-reply|create-issue)$/.test(path) && method === "POST") return false;
    if (/^\/api\/workspaces\/[^/]+\/feishu\/sources\/[^/]+\/status$/.test(path) && method === "GET") return false;
