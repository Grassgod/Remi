...
export class PasswordAccountsRepo {
  ...
  async login(email: string, password: string): Promise<{ user: MultiremiUser; token: string } | null> {
    ...
    const session = await this.accessTokens.createAccessToken({
      ...
      // Hold the credential row until token insertion commits. A concurrent
      // reset must either change the hash before this check or revoke the
      // newly inserted session after it, including on PostgreSQL.
      const current = this.db.query(
        `UPDATE multiremi_password_credentials SET updated_at = updated_at
         WHERE user_id = ? RETURNING password_hash`,
