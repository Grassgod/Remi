...
export class TasksRepo {
  ...
  getTaskWithAgent(id: string): MultiremiTaskWithAgent | null {
    ...
    return {
      ...
      repos: task.runtimeWorkspaceId || scheduleTarget || task.holdsWorkspace === false || (task.chatSessionId && !task.issueId && !project)
        ? []
  ...
  private refreshQueuedChatAffinity(workspaceId: string): void {
    ...
    for (const row of rows) {
      ...
      let runtimeId = affinity.runtimeId ?? (task.sessionId || resolveChatWorkspace(this.ctx, chat)?.mode === "managed" ? agent.runtimeId : task.runtimeId);
