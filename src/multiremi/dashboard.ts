export function renderMultiremiDashboardHtml(): string {
  return `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Bun Multiremi</title>
  <style>
    :root {
      color-scheme: light;
    ...
    * { box-sizing: border-box; }
    ...
    .brand-button:hover .multiremi-icon { animation: entrance-spin .6s ease-out both; }
...
<body>
  <div class="app">
    <aside class="sidebar">
      <div class="sidebar-header">
        <div class="brand-button">
          <span class="multiremi-icon"></span>
      ...
      <div class="nav-scroll">
        ...
        <div class="nav-group">
          ...
          <div class="nav-item" data-page="members"><span class="nav-icon"></span><span>Members</span></div>
    ...
    <main class="content">
      ...
      <section class="page" id="membersPage">
        <div class="collection">
          <div class="entity-grid" id="membersGrid"></div>
        </div>
      </section>
      ...
      <div class="floating-panel agent-sheet" id="agentSheet">
        ...
        <form class="sheet-form" id="agentForm">
          ...
          <label>Runtime<select id="agentRuntime"></select></label>
  ...
  <script>
    const pages = {
      ...
      members: { title: "Members", group: "Workspace" },
    ...
    document.getElementById("agentProvider").addEventListener("change", refreshAgentRuntimeOptions);
    ...
    async function refresh(options = {}) {
      ...
      try {
        const [agents, issues, tasks, runtimes, members, projects, squads, autopilots, skills, tokens, notifications, githubSettings, chats, inbox, labels, pins, usageDaily, usageByAgent, runtimeDaily] = await Promise.all([
          api("/api/multiremi/agents"),
          api("/api/multiremi/issues"),
          api("/api/multiremi/tasks"),
          api("/api/multiremi/runtimes"),
          api("/api/multiremi/members"),
          api("/api/multiremi/projects"),
          api("/api/multiremi/squads"),
          api("/api/multiremi/autopilots"),
          api("/api/multiremi/skills"),
          api("/api/multiremi/tokens"),
          api("/api/multiremi/notification-preferences"),
          api("/api/multiremi/github/settings"),
          api("/api/multiremi/chats"),
          api("/api/multiremi/inbox"),
          api("/api/multiremi/labels"),
          api("/api/multiremi/pins"),
        ...
        if (state.selectedMemberId) await loadMemberDetail(state.selectedMemberId, { silent: true });
    ...
    async function seed(provider) {
      const runtime = defaultRuntimeForProvider(provider);
      if (!runtime) {
        showNotice("Add a " + provider + " runtime first", els.agentNotice);
        return;
      }
      await api("/api/multiremi/agents/default", {
        method: "POST",
        body: JSON.stringify({
          provider,
    ...
    async function createAgent(event) {
      ...
      const body = {
        ...
        runtimeId: document.getElementById("agentRuntime").value,
      ...
      await api("/api/multiremi/agents", { method: "POST", body: JSON.stringify(body) });
    ...
    async function createTask(event) {
      ...
      try {
        ...
        const result = await api("/api/multiremi/issues", {
          method: "POST",
          body: JSON.stringify({
    ...
    async function postTask(body) {
      ...
      const result = await api("/api/multiremi/tasks", {
        method: "POST",
    ...
    async function cancelTask(id) {
      await api("/api/multiremi/tasks/" + encodeURIComponent(id) + "/cancel", { method: "POST" });
      await refresh();
    }
    ...
    async function openDaemonInstall() {
      state.selectedTaskId = null;
      ...
      els.taskDrawer.classList.add("open");
      renderDaemonInstallDrawer({ loading: true });
      try {
        const instructions = await api("/api/multiremi/install/daemon", {
          method: "POST",
          body: JSON.stringify({ workspaceId: "local" })
        });
        renderDaemonInstallDrawer({ instructions });
        await refresh({ silent: true });
      } catch (err) {
        renderDaemonInstallDrawer({ error: String(err.message || err) });
      }
    }
    ...
    async function openMember(id) {
      ...
      state.selectedAutopilotId = null;
      els.taskDrawer.classList.add("open");
      renderMemberDrawer({ loading: true });
    ...
    async function loadIssueDetail(id, options = {}) {
      try {
        const [issueResult, githubResult] = await Promise.all([
          api("/api/multiremi/issues/" + encodeURIComponent(id)),
          api("/api/multiremi/github/pull-requests?issueId=" + encodeURIComponent(id))
        ]);
    ...
    async function loadTaskDetail(id, options = {}) {
      try {
        const [taskResult, messageResult] = await Promise.all([
          api("/api/multiremi/tasks/" + encodeURIComponent(id)),
          api("/api/multiremi/tasks/" + encodeURIComponent(id) + "/messages")
        ]);
        state.selectedTask = taskResult.task;
        state.selectedMessages = messageResult.messages || [];
        if (state.selectedTask?.issueId) {
          const [issueResult, githubResult] = await Promise.all([
            api("/api/multiremi/issues/" + encodeURIComponent(state.selectedTask.issueId)),
            api("/api/multiremi/github/pull-requests?issueId=" + encodeURIComponent(state.selectedTask.issueId))
          ]);
    ...
    async function loadSquadDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/squads/" + encodeURIComponent(id));
    ...
    async function loadProjectDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/projects/" + encodeURIComponent(id));
    ...
    async function loadAgentDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/agents/" + encodeURIComponent(id));
    ...
    async function loadRuntimeDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/runtimes/" + encodeURIComponent(id));
    ...
    async function loadSkillDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/skills/" + encodeURIComponent(id));
    ...
    async function loadAutopilotDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/autopilots/" + encodeURIComponent(id));
    ...
    async function loadMemberDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/members/" + encodeURIComponent(id));
    ...
    async function loadChatDetail(id, options = {}) {
      try {
        const result = await api("/api/multiremi/chats/" + encodeURIComponent(id));
    ...
    async function updateSelectedAgent(event) {
      ...
      const tools = document.getElementById("agentAllowedTools").value
        ...
        .map(item => item.trim())
        .filter(Boolean);
      await api("/api/multiremi/agents/" + encodeURIComponent(state.selectedAgent.id), {
        method: "PATCH",
        body: JSON.stringify({
          name: document.getElementById("agentEditName").value,
          runtimeId: document.getElementById("agentEditRuntime").value,
    ...
    async function updateSelectedAgentSkills(event) {
      ...
      await api("/api/multiremi/agents/" + encodeURIComponent(state.selectedAgent.id) + "/skills", {
        method: "PUT",
    ...
    async function updateSelectedRuntime(event) {
      ...
      await api("/api/multiremi/runtimes/" + encodeURIComponent(state.selectedRuntime.id), {
        method: "PATCH",
        body: JSON.stringify({
    ...
    async function updateSelectedSkill(event) {
      ...
      await api("/api/multiremi/skills/" + encodeURIComponent(state.selectedSkill.id), {
        method: "PATCH",
        body: JSON.stringify({
    ...
    async function updateSelectedAutopilot(event) {
      ...
      await api("/api/multiremi/autopilots/" + encodeURIComponent(state.selectedAutopilot.id), {
        method: "PATCH",
        body: JSON.stringify({
    ...
    async function updateSelectedMember(event) {
      event.preventDefault();
      if (!state.selectedMember) return;
      await api("/api/multiremi/members/" + encodeURIComponent(state.selectedMember.id), {
        method: "PATCH",
        body: JSON.stringify({
          name: document.getElementById("memberEditName").value,
          email: document.getElementById("memberEditEmail").value || null,
          role: document.getElementById("memberEditRole").value || "member",
          workspaceId: document.getElementById("memberEditWorkspace").value || "local"
        })
      });
      await loadMemberDetail(state.selectedMember.id);
      await refresh({ silent: true });
    }
    ...
    async function addSquadMember(event) {
      ...
      if (!memberId) return;
      await api("/api/multiremi/squads/" + encodeURIComponent(state.selectedSquad.id) + "/members", {
        method: "POST",
    ...
    async function updateSelectedIssue() {
      ...
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id), {
        method: "PATCH",
        body: JSON.stringify({
    ...
    async function assignSelectedIssue(event) {
      ...
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/assign", {
        method: "POST",
    ...
    async function addSelectedIssueComment(event) {
      ...
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/comments", {
        method: "POST",
    ...
    async function addSelectedIssueDependency(event) {
      ...
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/dependencies", {
        method: "POST",
    ...
    async function deleteIssueDependency(dependencyId) {
      const issue = state.selectedIssue || state.selectedTask?.issue;
      if (!issue) return;
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/dependencies/" + encodeURIComponent(dependencyId), {
        method: "DELETE"
      });
      if (state.selectedIssueId) await loadIssueDetail(state.selectedIssueId);
      else await loadTaskDetail(state.selectedTaskId);
    ...
    async function reactToSelectedIssue(emoji) {
      const issue = state.selectedIssue || state.selectedTask?.issue;
      if (!issue) return;
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/reactions", {
        method: "POST",
    ...
    async function reactToComment(commentId, emoji) {
      await api("/api/multiremi/comments/" + encodeURIComponent(commentId) + "/reactions", {
        method: "POST",
        body: JSON.stringify({ actorType: "member", actorId: "local", emoji })
      });
      if (state.selectedIssueId) await loadIssueDetail(state.selectedIssueId);
    ...
    async function editComment(commentId) {
      ...
      await api("/api/multiremi/comments/" + encodeURIComponent(commentId), {
        method: "PUT",
        body: JSON.stringify({ body })
      });
      if (state.selectedIssueId) await loadIssueDetail(state.selectedIssueId);
    ...
    async function deleteComment(commentId) {
      ...
      await api("/api/multiremi/comments/" + encodeURIComponent(commentId), { method: "DELETE" });
      if (state.selectedIssueId) await loadIssueDetail(state.selectedIssueId);
      else await loadTaskDetail(state.selectedTaskId);
    }

    async function setCommentResolved(commentId, resolved) {
      await api("/api/multiremi/comments/" + encodeURIComponent(commentId) + "/resolve", {
    ...
    async function addSelectedIssueAttachment(event) {
      ...
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/attachments", {
        method: "POST",
        body: JSON.stringify({
          filename,
          url,
    ...
    async function attachSelectedIssueLabel(event) {
      ...
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/labels", {
        method: "POST",
    ...
    async function createSelectedIssueLabel(event) {
      ...
      const result = await api("/api/multiremi/labels", {
        method: "POST",
    ...
    async function detachSelectedIssueLabel(labelId) {
      const issue = state.selectedIssue || state.selectedTask?.issue;
      if (!issue || !labelId) return;
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/labels/" + encodeURIComponent(labelId), {
        method: "DELETE"
      });
      if (state.selectedIssueId) await loadIssueDetail(state.selectedIssueId);
      else await loadTaskDetail(state.selectedTaskId);
    ...
    async function setSelectedIssueMetadata(event) {
      ...
      await api("/api/multiremi/issues/" + encodeURIComponent(issue.id) + "/metadata/" + encodeURIComponent(key), {
        method: "PUT",
    ...
    function openEntitySheet(kind) {
      ...
      els.entitySheetTitle.textContent = kind === "project" ? "New project" : kind === "squad" ? "New squad" : kind === "skill" ? "New skill" : kind === "member" ? "New member" : "New autopilot";
    ...
    async function submitEntityForm(event) {
      ...
      try {
        if (kind === "project") {
          ...
          await api("/api/multiremi/projects", {
            method: "POST",
            body: JSON.stringify({
          ...
          await api("/api/multiremi/squads", {
            method: "POST",
            body: JSON.stringify({
          ...
          if (sourceUrl) {
            await api("/api/multiremi/skills/import", {
              method: "POST",
              body: JSON.stringify({
                url: sourceUrl,
            ...
            await api("/api/multiremi/skills", {
              method: "POST",
              body: JSON.stringify({
                name: document.getElementById("entityTitle").value,
                description: document.getElementById("entityDescription").value || "",
          ...
          await api("/api/multiremi/members", {
            method: "POST",
            body: JSON.stringify({
              name: document.getElementById("entityTitle").value,
              email: document.getElementById("entityEmail").value || null,
              role: document.getElementById("entityRole").value || "member",
              workspaceId: document.getElementById("entityWorkspace").value || "local"
            })
          });
        } else if (kind === "autopilot") {
          await api("/api/multiremi/autopilots", {
            method: "POST",
            body: JSON.stringify({
              title: document.getElementById("entityTitle").value,
              description: document.getElementById("entityDescription").value || null,
    ...
    async function runAutopilot(id) {
      try {
        const result = await api("/api/multiremi/autopilots/" + encodeURIComponent(id) + "/run", {
          method: "POST",
    ...
    async function replayWebhookDelivery(autopilotId, deliveryId) {
      try {
        await api("/api/multiremi/autopilots/" + encodeURIComponent(autopilotId) + "/deliveries/" + encodeURIComponent(deliveryId) + "/replay", {
          method: "POST"
        });
    ...
    async function archiveProject(id) {
      try {
        await api("/api/multiremi/projects/" + encodeURIComponent(id), { method: "DELETE" });
    ...
    async function addProjectResource(event) {
      ...
      if (!url) return;
      await api("/api/multiremi/projects/" + encodeURIComponent(state.selectedProject.id) + "/resources", {
        method: "POST",
        body: JSON.stringify({
          resourceType: "github_repo",
          resourceRef: {
    ...
    async function removeProjectResource(resourceId) {
      if (!state.selectedProject) return;
      await api("/api/multiremi/projects/" + encodeURIComponent(state.selectedProject.id) + "/resources/" + encodeURIComponent(resourceId), {
        method: "DELETE"
      });
      await loadProjectDetail(state.selectedProject.id);
      await refresh({ silent: true });
    ...
    async function unpinItem(itemType, itemId) {
      await api("/api/multiremi/pins/" + encodeURIComponent(itemType) + "/" + encodeURIComponent(itemId), {
        method: "DELETE"
      });
      await refresh({ silent: true });
      if (itemType === "issue" && state.selectedIssueId) await loadIssueDetail(state.selectedIssueId);
    ...
    async function archiveSquad(id) {
      try {
        await api("/api/multiremi/squads/" + encodeURIComponent(id), { method: "DELETE" });
        await refresh();
      } catch (err) {
        showNotice(String(err.message || err), els.notice);
      }
    ...
    async function setAutopilotStatus(id, status) {
      try {
        await api("/api/multiremi/autopilots/" + encodeURIComponent(id), {
          method: "PATCH",
    ...
    async function archiveAutopilot(id) {
      try {
        await api("/api/multiremi/autopilots/" + encodeURIComponent(id), { method: "DELETE" });
    ...
    async function archiveMember(id) {
      try {
        await api("/api/multiremi/members/" + encodeURIComponent(id), { method: "DELETE" });
        if (state.selectedMemberId === id) closeDrawer();
        await refresh();
      } catch (err) {
        showNotice(String(err.message || err), els.notice);
      }
    ...
    async function archiveAgent(id) {
      try {
        await api("/api/multiremi/agents/" + encodeURIComponent(id), { method: "DELETE" });
    ...
    async function archiveSkill(id) {
      try {
        await api("/api/multiremi/skills/" + encodeURIComponent(id), { method: "DELETE" });
    ...
    async function createToken(event) {
      ...
      if (!nameInput) return;
      try {
        const result = await api("/api/multiremi/tokens", {
          method: "POST",
          body: JSON.stringify({
    ...
    async function revokeToken(id) {
      await api("/api/multiremi/tokens/" + encodeURIComponent(id), { method: "DELETE" });
      await refresh({ silent: true });
    }
    ...
    async function updateNotificationPreferences(event) {
      ...
      const result = await api("/api/multiremi/notification-preferences", {
        method: "PUT",
    ...
    async function updateGitHubSettings(event) {
      ...
      const result = await api("/api/multiremi/github/settings", {
        method: "PUT",
    ...
    async function markInboxRead(id) {
      await api("/api/multiremi/inbox/" + encodeURIComponent(id) + "/read", { method: "POST" });
      await refresh({ silent: true });
    }

    async function archiveInbox(id) {
      await api("/api/multiremi/inbox/" + encodeURIComponent(id) + "/archive", { method: "POST" });
      await refresh({ silent: true });
    }
    ...
    async function submitChat(event) {
      ...
      try {
        ...
        const result = await api("/api/multiremi/chats/" + encodeURIComponent(session.id) + "/messages", {
          method: "POST",
    ...
    async function ensureChatSession(agentId, firstPrompt) {
      ...
      const result = await api("/api/multiremi/chats", {
        method: "POST",
        body: JSON.stringify({
          agentId,
          workspaceId: "local",
    ...
    function renderShell() {
      ...
      els.membersPage.classList.toggle("active", state.page === "members");
      ...
      const isPlaceholder = !["issues", "my-issues", "agents", "projects", "autopilots", "members", "squads", "runtimes", "skills", "settings", "inbox", "usage"].includes(state.page);
    ...
    function renderToolbar() {
      if (state.page === "issues") {
      ...
      } else if (state.page === "members") {
        const assigned = state.issues.filter(issue => issue.assigneeType === "member").length;
        els.toolbar.innerHTML =
          "<div class=\\"toolbar-left\\"><span class=\\"status-badge\\">" + state.members.length + " members</span><span class=\\"status-badge\\">" + assigned + " assigned issues</span></div>" +
          "<div class=\\"toolbar-right\\"><button class=\\"chip-button\\" id=\\"refresh\\">Refresh</button><button class=\\"primary\\" id=\\"newMember\\">New member</button></div>";
        document.getElementById("refresh").addEventListener("click", () => refresh());
        document.getElementById("newMember").addEventListener("click", () => openEntitySheet("member"));
        ...
        els.toolbar.innerHTML =
          ...
          "</div>" +
          "<div class=\\"toolbar-right\\"><button class=\\"chip-button\\" id=\\"refresh\\">Refresh</button><button class=\\"primary\\" id=\\"addComputer\\">Add computer</button></div>";
        document.getElementById("refresh").addEventListener("click", () => refresh());
        document.getElementById("addComputer").addEventListener("click", openDaemonInstall);
    ...
    function visibleTasks() {
      ...
      if (state.activeOnly) {
        tasks = tasks.filter(issue => ["todo", "open", "in_progress", "blocked"].includes(issue.status));
      }
    ...
    function visibleMyIssues() {
      ...
      if (state.myIssuesActiveOnly) {
        issues = issues.filter(issue => ["todo", "open", "in_progress", "blocked"].includes(issue.status));
      }
    ...
    function renderBoard() {
      const groups = [
        { key: "todo", title: "Todo", test: t => t.status === "todo" || t.status === "open" },
    ...
    function renderMyIssues() {
      ...
      const active = scopedMemberIssues.filter(issue => ["todo", "open", "in_progress", "blocked"].includes(issue.status));
    ...
    function renderMembers() {
      if (!state.members.length) {
        els.membersGrid.innerHTML = "<div class=\\"empty-column\\">No members</div>";
        return;
      }
      els.membersGrid.innerHTML = state.members.map(member => {
        const assigned = state.issues.filter(issue => issue.assigneeType === "member" && issue.assigneeId === member.id);
        const active = assigned.filter(issue => ["todo", "open", "in_progress", "blocked"].includes(issue.status)).length;
        const runtimes = state.runtimes.filter(runtime => runtime.ownerId === member.id).length;
        return "<article class=\\"entity-card\\" onclick=\\"openMember('" + escAttr(member.id) + "')\\">" +
          "<div class=\\"entity-head\\">" +
            "<span class=\\"agent-avatar\\">" + esc(initials(member.name)) + "</span>" +
            "<div class=\\"entity-main\\"><div class=\\"entity-title\\">" + esc(member.name) + "</div><div class=\\"entity-subtitle\\">" + esc(member.role || "member") + " / " + esc(member.workspaceId || "local") + "</div></div>" +
          "</div>" +
          "<div class=\\"entity-body\\">" + esc(member.email || "No email") + "</div>" +
          "<div class=\\"metric-row\\">" +
            renderMetric(assigned.length, "assigned") +
            renderMetric(active, "active") +
            renderMetric(runtimes, "runtimes") +
          "</div>" +
          "<button class=\\"destructive\\" onclick=\\"event.stopPropagation(); archiveMember('" + escAttr(member.id) + "')\\">Archive</button>" +
        "</article>";
      }).join("");
    }
    ...
    function renderRuntimes() {
      if (!state.runtimes.length) {
        els.runtimesGrid.innerHTML = "<div class=\\"empty-column\\">No runtimes yet<br><button class=\\"outline\\" onclick=\\"openDaemonInstall()\\">Add computer</button></div>";
        return;
      }
    ...
    function renderSettings() {
      ...
      els.settingsGrid.innerHTML =
        "<article class=\\"entity-card\\">" +
          "<div class=\\"entity-head\\"><span class=\\"agent-avatar\\">S</span><div class=\\"entity-main\\"><div class=\\"entity-title\\">Server</div><div class=\\"entity-subtitle\\">Bun Multiremi local API</div></div></div>" +
        ...
        "<div class=\\"drawer-body\\">" +
          ...
          "<form class=\\"sheet-form\\" onsubmit=\\"updateSelectedAgent(event)\\" style=\\"padding:0;\\">" +
            "<div class=\\"detail-grid\\">" +
              ...
              "<label>Runtime<select id=\\"agentEditRuntime\\">" + agentRuntimeOptions(agent.provider, agent.runtimeId || agent.runtime_id || "") + "</select></label>" +
    ...
    function renderDaemonInstallDrawer(options = {}) {
      if (options.loading) {
        els.taskDrawer.innerHTML =
          "<div class=\\"drawer-head\\"><div class=\\"drawer-title\\"><strong>Add computer</strong><span>Multiremi daemon</span></div><button class=\\"icon\\" onclick=\\"closeDrawer()\\">x</button></div>" +
          "<div class=\\"drawer-body\\"><div class=\\"empty-column\\">Preparing commands</div></div>";
        return;
      }
      if (options.error) {
        els.taskDrawer.innerHTML =
          "<div class=\\"drawer-head\\"><div class=\\"drawer-title\\"><strong>Add computer</strong><span>Multiremi daemon</span></div><button class=\\"icon\\" onclick=\\"closeDrawer()\\">x</button></div>" +
          "<div class=\\"drawer-body\\"><div class=\\"empty-column\\">" + esc(options.error) + "</div></div>";
        return;
      }
      const instructions = options.instructions || {};
      const commands = instructions.commands || [];
      els.taskDrawer.innerHTML =
        "<div class=\\"drawer-head\\">" +
          "<div class=\\"drawer-title\\"><strong>Add computer</strong><span>Multiremi daemon</span></div>" +
          "<button class=\\"icon\\" onclick=\\"closeDrawer()\\">x</button>" +
        "</div>" +
        "<div class=\\"drawer-body\\">" +
          "<div class=\\"detail-grid\\">" +
            renderCell("Server", instructions.serverUrl || "") +
            renderCell("Workspace", instructions.workspaceId || "local") +
            renderCell("Provider", instructions.provider || "auto") +
            renderCell("Token", instructions.tokenId ? shortId(instructions.tokenId) : "provided") +
          "</div>" +
          "<div class=\\"detail-block\\"><div class=\\"detail-label\\">Commands</div><div class=\\"message-list\\">" +
            commands.map(renderDaemonInstallCommand).join("") +
          "</div></div>" +
        "</div>";
    }

    function renderDaemonInstallCommand(command) {
      return "<div class=\\"message-row\\">" +
        "<div class=\\"message-head\\"><span>" + esc(command.label || command.key || "Command") + "</span></div>" +
        "<div class=\\"message-content\\">" + esc(command.command || "") + "</div>" +
      "</div>";
    }
    ...
    function renderAutopilotDrawer(options = {}) {
      ...
      const webhookUrl = window.location.origin + "/api/multiremi/autopilots/" + encodeURIComponent(autopilot.id) + "/webhook";
      els.taskDrawer.innerHTML =
        "<div class=\\"drawer-head\\">" +
    ...
    function renderMemberDrawer(options = {}) {
      if (options.loading || !state.selectedMember) {
        els.taskDrawer.innerHTML =
          "<div class=\\"drawer-head\\"><div class=\\"drawer-title\\"><strong>Loading</strong><span>Member detail</span></div><button class=\\"icon\\" onclick=\\"closeDrawer()\\">x</button></div>" +
          "<div class=\\"drawer-body\\"><div class=\\"empty-column\\">Loading</div></div>";
        return;
      }
      const member = state.selectedMember;
      const assigned = state.issues.filter(issue => issue.assigneeType === "member" && issue.assigneeId === member.id);
      const active = assigned.filter(issue => ["todo", "open", "in_progress", "blocked"].includes(issue.status));
      const runtimes = state.runtimes.filter(runtime => runtime.ownerId === member.id);
      els.taskDrawer.innerHTML =
        "<div class=\\"drawer-head\\">" +
          "<div class=\\"drawer-title\\"><strong>" + esc(member.name || "") + "</strong><span>" + esc(member.role || "member") + " / " + esc(shortId(member.id)) + "</span></div>" +
          "<button class=\\"destructive\\" onclick=\\"archiveMember('" + escAttr(member.id) + "')\\">Archive</button>" +
          "<button class=\\"icon\\" onclick=\\"closeDrawer()\\">x</button>" +
        "</div>" +
        "<div class=\\"drawer-body\\">" +
          "<div class=\\"metric-row\\">" +
            renderMetric(assigned.length, "assigned") +
            renderMetric(active.length, "active") +
            renderMetric(runtimes.length, "runtimes") +
          "</div>" +
          "<form class=\\"sheet-form\\" onsubmit=\\"updateSelectedMember(event)\\" style=\\"padding:0;\\">" +
            "<div class=\\"detail-grid\\">" +
              "<label>Name<input id=\\"memberEditName\\" value=\\"" + escAttr(member.name || "") + "\\" required></label>" +
              "<label>Email<input id=\\"memberEditEmail\\" type=\\"email\\" value=\\"" + escAttr(member.email || "") + "\\" placeholder=\\"name@example.com\\"></label>" +
              "<label>Role<select id=\\"memberEditRole\\">" + memberRoleOptions(member.role) + "</select></label>" +
              "<label>Workspace<input id=\\"memberEditWorkspace\\" value=\\"" + escAttr(member.workspaceId || "local") + "\\"></label>" +
            "</div>" +
            "<button class=\\"outline\\" type=\\"submit\\">Save member</button>" +
          "</form>" +
          "<div class=\\"detail-block\\"><div class=\\"detail-label\\">Assigned issues</div><div class=\\"message-list\\">" + renderMemberIssues(assigned) + "</div></div>" +
          "<div class=\\"detail-block\\"><div class=\\"detail-label\\">Owned runtimes</div><div class=\\"message-list\\">" + renderMemberRuntimes(runtimes) + "</div></div>" +
        "</div>";
    }

    function renderMemberIssues(issues) {
      if (!issues.length) return "<div class=\\"empty-column\\">No assigned issues</div>";
      return issues.map(issue =>
        "<div class=\\"message-row\\" onclick=\\"openIssue('" + escAttr(issue.id) + "')\\">" +
          "<div class=\\"message-head\\"><span>" + esc(issueLabel(issue)) + "</span><span>" + esc(statusLabel(issue.status)) + "</span><span>" + esc(issue.priority || "none") + "</span></div>" +
          "<div class=\\"message-content\\">" + esc(issue.title || "") + "</div>" +
        "</div>"
      ).join("");
    }

    function renderMemberRuntimes(runtimes) {
      if (!runtimes.length) return "<div class=\\"empty-column\\">No owned runtimes</div>";
      return runtimes.map(runtime =>
        "<div class=\\"message-row\\" onclick=\\"openRuntime('" + escAttr(runtime.id) + "')\\">" +
          "<div class=\\"message-head\\"><span>" + esc(runtime.name) + "</span><span>" + esc(runtime.provider) + "</span><span>" + esc(runtime.status) + "</span></div>" +
          "<div class=\\"message-content\\">" + esc(String(runtime.activeTaskCount || 0)) + " active / " + esc(String(runtime.maxConcurrency || 1)) + " capacity</div>" +
        "</div>"
      ).join("");
    }
    ...
    function renderSearchResults() {
      ...
      pages.members && rows.push({ type: "Page", title: "Members", subtitle: state.members.length + " members", action: () => switchPage("members") });
      ...
      state.members.forEach(m => rows.push({ type: "Member", title: m.name, subtitle: m.role + " / " + m.workspaceId, action: () => { switchPage("members"); openMember(m.id); } }));
    ...
    function renderChat() {
      const entries = state.selectedChatMessages.length
        ? state.selectedChatMessages.map(message => ({
          role: message.role,
          text: message.body,
          label: message.role === "user" ? "You" : message.role === "assistant" ? "Agent" : "Multiremi"
        }))
        : state.chatEntries;
      if (!entries.length) {
        ...
        els.chatLog.innerHTML = latest
          ? "<div class=\\"chat-entry system\\"><span>Multiremi</span><div>Open chat: " + esc(latest.title) + "</div></div>"
          : "<div class=\\"chat-entry system\\"><span>Multiremi</span><div>Ready</div></div>";
        return;
      }
      els.chatLog.innerHTML = entries.slice(-12).map(entry =>
        "<div class=\\"chat-entry " + esc(entry.role) + "\\"><span>" + esc(entry.label || (entry.role === "user" ? "You" : "Multiremi")) + "</span><div>" + esc(entry.text) + "</div></div>"
      ).join("");
    }
    ...
    function entityFormHtml(kind) {
      ...
      if (kind === "member") {
        return "<label>Name<input id=\\"entityTitle\\" required placeholder=\\"Member name\\"></label>" +
          "<label>Email<input id=\\"entityEmail\\" type=\\"email\\" placeholder=\\"name@example.com\\"></label>" +
          "<label>Role<select id=\\"entityRole\\">" + memberRoleOptions("member") + "</select></label>" +
          "<label>Workspace<input id=\\"entityWorkspace\\" value=\\"local\\"></label>" +
          "<button class=\\"primary\\" type=\\"submit\\">Create member</button><div class=\\"notice\\"></div>";
      }
    ...
    function memberRoleOptions(current) {
      const roles = ["member", "admin", "reviewer", "observer"];
      return roles.map(role =>
        "<option value=\\"" + role + "\\" " + (role === (current || "member") ? "selected" : "") + ">" + role + "</option>"
      ).join("");
    }
    ...
    function agentRuntimeOptions(provider, current) {
      const runtimes = state.runtimes.filter(runtime => !provider || runtime.provider === provider || runtime.provider === "any");
      if (!runtimes.length) {
        return "<option value=\\"\\">No matching runtime</option>";
      }
      return runtimes.map(runtime =>
        "<option value=\\"" + escAttr(runtime.id) + "\\" " + (runtime.id === current ? "selected" : "") + ">" +
          esc(runtime.name || runtime.id) + " / " + esc(runtime.provider) + " / " + esc(runtime.status) +
        "</option>"
      ).join("");
    }

    function defaultRuntimeForProvider(provider) {
      return state.runtimes.find(runtime => runtime.provider === provider && runtime.status === "online")
        || state.runtimes.find(runtime => runtime.provider === provider)
        || state.runtimes.find(runtime => runtime.provider === "any")
        || null;
    }

    function refreshAgentRuntimeOptions() {
      const select = document.getElementById("agentRuntime");
      const provider = document.getElementById("agentProvider")?.value || "";
      if (!select) return;
      const current = select.value;
      select.innerHTML = agentRuntimeOptions(provider, current);
    }
    ...
    function issueStatusOptions(current) {
      return ["backlog", "todo", "in_progress", "in_review", "blocked", "done", "cancelled"].map(status =>
        "<option value=\\"" + status + "\\" " + (status === current ? "selected" : "") + ">" + status + "</option>"
      ).join("");
    }
    ...
    window.openDaemonInstall = openDaemonInstall;
    ...
    window.archiveMember = archiveMember;
    ...
    window.updateSelectedMember = updateSelectedMember;
