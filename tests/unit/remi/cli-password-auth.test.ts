...
describe("password authentication CLI", () => {
  ...
  it("does not forward saved or explicit access credentials to the public login endpoint", async () => {
    writeFileSync(configPath, JSON.stringify({ server_url: "https://previous.example.test", token: "previous-server-session" }));
    process.env.MULTIREMI_TOKEN = "previous-environment-credential";
    globalThis.fetch = (async (input, init) => {
      const request = new Request(input, init);
      expect(request.url).toBe("https://next.example.test/auth/password");
      expect(request.headers.get("Authorization")).toBeNull();
      expect(await request.json()).toEqual(credentials);
      return Response.json({ token: "next-server-session", user: { id: "usr_next", email: credentials.email } });
    }) as typeof fetch;
    await registry().execute([
      "context", "auth", "password", "--file", inputPath,
      "--server", "https://next.example.test", "--token", "explicit-old-credential",
    ]);
    expect(JSON.parse(readFileSync(configPath, "utf8")).token).toBe("next-server-session");
    expect(output.join("\n")).not.toMatch(/previous-server-session|previous-environment-credential|explicit-old-credential|next-server-session/);
  });
  ...
  it("negotiates account provisioning and forwards the master credential with file input", async () => {
    ...
    globalThis.fetch = (async (input, init) => {
      ...
      expect(request.method).toBe("POST");
      expect(await request.json()).toEqual({ ...credentials, name: "CLI Reader", workspaceId: "ws_explicit", workspace_id: "ws_explicit" });
