...
describe("Bun Multiremi API", () => {
  ...
  it("seeds Remi builtin skills into workspace skills", async () => {
    const store = createStore();
    const app = createMultiremiApp({ store });

    const builtinList = await app.request("/api/multiremi/skills/builtin?workspace_id=team");
    expect(builtinList.status).toBe(200);
    const builtinBody = await builtinList.json();
    expect(builtinBody.total).toBe(7);
    expect(builtinBody.skills.map((skill: any) => skill.name).sort()).toEqual([
      "contract-eval",
      "daily-changelog",
      "execute",
      "intake",
    ...
    expect(builtinBody.skills.find((skill: any) => skill.name === "intake").content).toBeUndefined();
    expect(builtinBody.skills.find((skill: any) => skill.name === "intake").config.origin).toMatchObject({
      type: "remi_builtin",
      slug: "intake",
      source_path: "pipeline/skills/intake/SKILL.md",
    });

    const seeded = await app.request("/api/multiremi/skills/builtin/seed", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ workspace_id: "team", created_by: "usr_seed" }),
    });
    expect(seeded.status).toBe(200);
    expect((await seeded.json()).total).toBe(7);

    const intake = store.getSkill("skl_remi_builtin_intake");
    expect(intake).toMatchObject({
      id: "skl_remi_builtin_intake",
      workspaceId: "team",
      name: "intake",
      createdBy: "usr_seed",
    });
    expect(intake?.content).toContain("# 需求澄清 Skill");
    expect(intake?.config?.origin).toMatchObject({
      type: "remi_builtin",
      slug: "intake",
      source_path: "pipeline/skills/intake/SKILL.md",
    });
    expect(String((intake?.config?.origin as any)?.content_sha256)).toHaveLength(64);

    const seededAgain = await app.request("/api/multiremi/skills/builtin/seed", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ workspace_id: "team", created_by: "usr_seed" }),
    });
    expect(seededAgain.status).toBe(200);
    expect(store.listSkills("team").filter((skill) => (skill.config?.origin as { type?: string } | undefined)?.type === "remi_builtin")).toHaveLength(7);

    store.archiveSkill("skl_remi_builtin_intake");
    expect(store.getSkill("skl_remi_builtin_intake")).toBeNull();
    await app.request("/api/multiremi/skills/builtin/seed", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ workspace_id: "team", created_by: "usr_seed" }),
    });
    expect(store.getSkill("skl_remi_builtin_intake")?.archivedAt).toBeNull();
  });
