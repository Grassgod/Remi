...
describe("persisted inherited context diagnostics", () => {
  it("upgrades existing tasks with five nullable columns and preserves null through the mapper", () => {
    ...
    for (const column of diagnosticColumns) {
      expect(columns).toContainEqual(expect.objectContaining({ name: column, type: "INTEGER", notnull: 0, dflt_value: null }));
    }
  ...
  it("reports null before claim, then the actual inherited claim values rather than the untruncated own projection", async () => {
    ...
    expect(context.diagnostics).toEqual({
      ...
      estimated_tokens: inherited.estimated_tokens, token_budget: budget, recorded_at: stored.updatedAt,
    });
  });
  ...
  it("keeps all five fields null for an ordinary task and returns a successful none response", async () => {
  ...
  it("selects the latest updated task with inherited diagnostics in this Session, skipping newer unclaimed tasks", async () => {
    const { store, agent, issue, parent, side, task, app } = fixture();
    ...
    db!.run("UPDATE multiremi_tasks SET updated_at = ? WHERE id = ?", ["2026-09-17T02:00:00.000Z", second.id]);
    db!.run("UPDATE multiremi_tasks SET updated_at = ? WHERE id = ?", [recordedAt, task.id]);
    db!.run("UPDATE multiremi_tasks SET updated_at = ? WHERE id IN (?, ?)", ["2026-09-17T04:00:00.000Z", pending.id, otherTask.id]);
    const response = await app.request(`/api/sessions/${side.id}/inherited-context`, { headers });
    expect(response.status).toBe(200);
