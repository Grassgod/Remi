...
describe("password accounts", () => {
  ...
  it("stores only a private Argon2id hash and returns the real session identity after reload", async () => {
    ...
    const loggedOut = await app.request("/auth/logout", { method: "POST", headers: { Cookie: cookie.split(";")[0]! } });
    expect(loggedOut.status).toBe(200);
    expect(loggedOut.headers.get("set-cookie")).toContain("multimira_auth=;");
    expect(await store.verifyAccessToken(session.token)).toBeNull();
    expect((await app.request("/api/me", { headers: { Cookie: cookie.split(";")[0]! } })).status).toBe(401);
  });
