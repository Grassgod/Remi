...
describe("local stable and dev isolation", () => {
  test("uses real Node for Next.js development and build while pinning the Bun installer", () => {
    const dockerfile = readFileSync(resolve(repoRoot, "deploy/docker/Dockerfile.web"), "utf8");
    expect(dockerfile).toMatch(/^FROM oven\/bun:1\.3\.14 AS bun$/mu);
    expect(dockerfile).toMatch(/^FROM node:22-bookworm-slim AS dependencies$/mu);
    expect(dockerfile).toContain("COPY --from=bun /usr/local/bin/bun /usr/local/bin/bun");
    expect(dockerfile).toMatch(/^FROM dependencies AS development$/mu);
    expect(dockerfile).toMatch(/^FROM dependencies AS builder$/mu);
