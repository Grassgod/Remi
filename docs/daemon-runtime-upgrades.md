...
daemon 由 systemd 托管时，重启交给 `systemctl --user restart --no-block <unit>`，由 systemd 重建整个 unit cgroup，只留一个新进程；launchd 托管时使用 `launchctl kickstart -k`。unit 名只从 /proc/self/cgroup 的 `0::` 或 `name=systemd` 行取，cgroup v1 主机上其它控制器行只到 `user@<uid>.service`（见 [apps/remi/cli/multiremi/service.ts](../apps/remi/cli/multiremi/service.ts)）。管理器调用失败或进程不受管理器托管时才退回 spawn 后继进程。修复前的版本在 cgroup v1 主机上总是退回 spawn，旧进程留在 unit 里；这样启动的新进程在接管工作区和接单前自检：自己不是 unit 的 MainPID、MainPID 是前台 daemon、且本用户没有其它 daemon 持有工作区 supervisor 租约时，请求一次 unit 重启。租约仍被占用说明有 daemon 可能在跑任务，或者自己只是任务里继承了 INVOCATION_ID 的进程，此时只记日志不重启。
