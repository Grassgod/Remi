...
云友的「执行目标」当前按机器与 Runtime 类型选择，使用 `runtime_id` 引用对应 Runtime，`provider` 表示引擎类型；不引入能力标签或分组管理。创建和编辑共用选择器，模型及思考选项来自该目标的有效连接目录（自定义连接优先，否则应用工作区网关），在线数量也限定到该目标。领取及重领通过 `runtimeCanRunAgent` 检查绑定、类型、工作区和归属；目标离线时排队，不转到其他同引擎机器。项目设备和本机目录约束继续取交集。

切换执行目标默认清空模型和思考覆盖，未冻结的排队任务清除旧会话信息，已冻结但未启动的任务取消，运行中任务继续原执行。绑定跨服务重启保留；普通 Runtime 删除不能静默解除云友绑定。已有未绑定云友保留原调度行为，选择目标后开始遵守绑定。前端统一执行目标组件与后端匹配入口是后续扩展位置，目前只实现机器与类型。
