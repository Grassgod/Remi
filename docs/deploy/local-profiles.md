...
这里的 `~` 是当前用户目录；Windows PowerShell 中可通过 `$env:USERPROFILE` 获取。配置、源码快照和备份在该目录；数据库与运行文件保存在 Docker 管理的上述独立 Linux 卷中，不是仓库内的普通目录。
...
在主机选择实际的 RFC 1918 内网 IPv4 地址，然后部署；将下面的占位值替换为该地址：

```powershell
node scripts/local-profile.mjs stable deploy --ref HEAD --lan-host <内网IPv4>
```
...
脚本与配置隔离测试：`node --test scripts/local-profile.test.mjs` 和 `bun test tests/arch/deploy-local-profiles.test.ts`。
`bun run smoke:local-auth` 使用隔离随机账号、真实 Next/API 与临时 SQLite 验证登录、刷新、退出和 CLI 回调；不读取个人 profile。
密码存储与权限测试见 [password-auth.test.ts](../../tests/unit/multiremi/password-auth.test.ts)，
PostgreSQL 并发重置测试见 [password-auth-postgres.test.ts](../../tests/unit/multiremi/password-auth-postgres.test.ts)，后者仅在显式设置 `MULTIREMI_TEST_POSTGRES_URL` 指向可创建测试数据库的隔离实例时运行。
实际部署版本、迁移备份和本次测试结果记录在任务中，本页只维护操作方法。
