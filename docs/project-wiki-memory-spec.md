...
- **测试装置**（照 tests/unit/multiremi/multiremi-core.test.ts:1-90 抄）：`db = new Database(":memory:"); new MultiremiStore(db)`；API 测试用 `createMultiremiApp`（`@multiremi/api.js`）；member 鉴权用 `signTestJwt(payload)`（HS256，dev secret `multiremi-dev-secret-change-in-production`）；`mockFetch`/`jsonResponse` helper 现成。`buildTaskPrompt` 从 `@multiremi/prompt.js` 导出并已在该文件被测——daemon 注入断言可直接加在同文件或新建同装置文件。
