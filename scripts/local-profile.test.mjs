...
fs.writeFileSync = (path, value, options) => {
  if (process.env.TEST_FAIL === 'write-env' && !failedWrite && /compose\.env(?:\.tmp-|$)/u.test(String(path))) {
    failedWrite = true;
    originalWrite(path, 'Incomplete write', options);
...
test('an interrupted environment write preserves the previous recoverable deployment', (t) => {
  const f = fixture(t);
  succeeds(f.run('stable', 'prepare'));
  const names = ['deployment.json', 'compose.env', 'compose.yml'];
  const before = new Map(names.map((name) => [name, readFileSync(join(f.profileRoot(), name))]));
  const result = f.run('stable', 'prepare', { args: ['--ref', 'new-ref'], commit: NEW_REF, fail: 'write-env' });
  assert.notEqual(result.status, 0);
  for (const [name, contents] of before) {
    assert.ok(readFileSync(join(f.profileRoot(), name)).equals(contents), `${name} was not restored after a write failure`);
  }
  assert.ok(!readdirSync(f.profileRoot()).some((name) => name.includes('.tmp-')));
});

test('malformed credential files report a safe error without printing their content', (t) => {
  const f = fixture(t);
  succeeds(f.run('dev', 'prepare'));
  const privateMarker = 'PRIVATE';
  writeFileSync(join(f.profileRoot('dev'), 'credentials.json'), `${privateMarker}-invalid-json`);
  const result = f.run('dev', 'prepare');
  assert.notEqual(result.status, 0);
  assert.ok(!result.stdout.includes(privateMarker) && !result.stderr.includes(privateMarker), 'A parser error disclosed credential-file content');
});

test('generated credentials are distinct, retained across prepare, private on POSIX, and absent from regular output', (t) => {
  const f = fixture(t);
  const stable = f.run('stable', 'prepare');
  const dev = f.run('dev', 'prepare');
  succeeds(stable);
  succeeds(dev);
  const values = [];
  for (const profile of ['stable', 'dev']) {
    const root = f.profileRoot(profile);
    const environment = parseEnv(readFileSync(join(root, 'api.env'), 'utf8'));
    values.push(f.readProfile('credentials.json', profile).databasePassword);
    values.push(environment.MULTIREMI_TOKEN, environment.JWT_SECRET, environment.MULTIREMI_SHARE_SECRET);
    assert.equal(Buffer.from(environment.MULTIREMI_SCM_ENCRYPTION_KEY, 'base64').length, 32);
    if (process.platform !== 'win32') {
      assert.equal(statSync(root).mode & 0o077, 0);
      for (const name of ['credentials.json', 'api.env', 'compose.env', 'deployment.json']) {
        assert.equal(statSync(join(root, name)).mode & 0o077, 0, `${name} is accessible by another user`);
      }
    }
  }
  assert.ok(values.every(value => /^[a-f0-9]{64}$/u.test(value)));
  assert.equal(new Set(values).size, values.length);
  assert.ok(values.every(value => !`${stable.stdout}${stable.stderr}${dev.stdout}${dev.stderr}`.includes(value)));
  const before = readFileSync(join(f.profileRoot(), 'api.env'));
  succeeds(f.run('stable', 'prepare'));
  assert.ok(readFileSync(join(f.profileRoot(), 'api.env')).equals(before), 'Preparing again rotated the existing credentials');
});
