...
import { existsSync, mkdirSync, readFileSync, writeFileSync, copyFileSync, openSync, closeSync, realpathSync, renameSync, unlinkSync } from 'node:fs';
...
function readJson(path) {
  ...
  catch { throw new Error(`Invalid JSON in profile file ${basename(path)}`); }
}
...
function writeComposeEnvironment(root, deployment) {
  ...
  writePrivateFile(join(root, 'compose.env'), Object.entries(variables).map(([key, value]) => `${key}=${envValue(value)}`).join('\n') + '\n');
}
...
function prepare(profile, ref, lanHost) {
  ...
  try {
    ...
    for (const [name, contents] of original) {
      if (contents) writePrivateFile(join(root, name), contents);
...
async function main() {
  ...
  if (action === 'prepare') {
    ...
    try {
      ...
      for (const [name, contents] of oldFiles) writePrivateFile(join(root, name), contents);
      throw error;
    }
    let saved;
    if (previous) {
      ...
      for (const [name, contents] of candidate) writePrivateFile(join(root, name), contents);
    }
